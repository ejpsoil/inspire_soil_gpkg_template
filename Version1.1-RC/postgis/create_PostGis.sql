...
BEGIN
    IF NEW.soilinvestigationpurpose NOT IN (SELECT id FROM so.codelist WHERE collection = 'SoilInvestigationPurposeValue') THEN
        RAISE EXCEPTION 'Table soilsite: Invalid value for soilinvestigationpurpose. Must be present in id of soilinvestigationpurposevalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.soilplottype NOT IN (SELECT id FROM so.codelist WHERE collection = 'SoilPlotTypeValue') THEN
        RAISE EXCEPTION 'Table soilplot: Invalid value for soilplottype. Must be present in id of soilplottypevalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'Table soilprofile: Invalid value for wrbversion. Must be present in id of the correct yearso.codelist collection.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF (NEW.wrbversion NOT IN (SELECT id FROM so.codelist WHERE collection = 'wrbversion') AND NEW.wrbversion IS NOT NULL) THEN
        RAISE EXCEPTION 'Table soilprofile: Invalid value for wrbversion. Must be present in id of wrbreferencesoilgroupvalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.othersoilname_type NOT IN (SELECT id FROM so.codelist WHERE collection = 'OtherSoilNameTypeValue') THEN
        RAISE EXCEPTION 'Table othersoilnametype: Invalid value for othersoilname_type. Must be present in id of othersoilnametypevalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.layergenesisenviroment IS NOT NULL AND NOT EXISTS (SELECT 1 FROM so.codelist WHERE id = NEW.layergenesisenviroment AND collection = 'EventEnvironmentValue') THEN
        RAISE EXCEPTION 'Table profileelement: Invalid value for layergenesisenviroment. Must be present in id of eventenvironmentvalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.layergenesisprocess IS NOT NULL AND NOT EXISTS (SELECT 1 FROM so.codelist WHERE id = NEW.layergenesisprocess AND collection = 'EventProcessValue') THEN
        RAISE EXCEPTION 'Table profileelement: Invalid value for layergenesisprocess. Must be present in id of eventprocessvalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.layergenesisprocessstate IS NOT NULL AND NOT EXISTS (SELECT 1 FROM so.codelist WHERE id = NEW.layergenesisprocessstate AND collection = 'LayerGenesisProcessStateValue') THEN
        RAISE EXCEPTION 'Table profileelement: Invalid value for layergenesisprocessstate. Must be present in id of layergenesisprocessstatevalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.layerrocktype IS NOT NULL AND NOT EXISTS (SELECT 1 FROM so.codelist WHERE id = NEW.layerrocktype AND collection = 'LithologyValue') THEN
        RAISE EXCEPTION 'Table profileelement: Invalid value for layerrocktype. Must be present in id of lithologyvalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.faohorizonmaster_1 IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM so.codelist WHERE id = NEW.faohorizonmaster_1 AND collection = 'FAOHorizonMasterValue') THEN
            RAISE EXCEPTION 'Table faohorizonnotationtype: Invalid value for faohorizonmaster. Must be present in id of faohorizonmastervalueso.codelist.';
        END IF;
    END IF;
    RETURN NEW;
END;
...
BEGIN
    IF NEW.faohorizonsubordinate_1 IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM so.codelist WHERE id = NEW.faohorizonsubordinate_1 AND collection = 'FAOHorizonSubordinateValue') THEN
            RAISE EXCEPTION 'Table faohorizonnotationtype: Invalid value for faohorizonsubordinate. Must be present in id of faohorizonsubordinatevalueso.codelist.';
        END IF;
    END IF;
    RETURN NEW;
END;
...
BEGIN
    IF NEW.faoprime IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM so.codelist WHERE id = NEW.faoprime AND collection = 'FAOPrimeValue') THEN
            RAISE EXCEPTION 'Table faohorizonnotationtype: Invalid value for faoprime. Must be present in id of faoprimevalueso.codelist.';
        END IF;
    END IF;
    RETURN NEW;
END;
...
BEGIN
    IF NEW.horizonnotation IS NOT NULL AND NEW.horizonnotation NOT IN (SELECT id FROM so.codelist WHERE collection = 'OtherHorizonNotationTypeValue') THEN
        RAISE EXCEPTION 'Invalid value for horizonnotation in table otherhorizonnotationtype. Must be present in id of otherhorizonnotationtypevalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'Table wrbqualifiergrouptype: Invalid value for wrbversion. Must be present in id of the correct yearso.codelist collection.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NOT EXISTS (SELECT id FROM so.codelist WHERE collection = 'WRBQualifierPlaceValue' AND id = NEW.qualifierplace) THEN
        RAISE EXCEPTION 'Table wrbqualifiergrouptype: Invalid value for qualifierplace. Must be present in id of wrbqualifierplacevalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF (NEW.wrbversion NOT IN (SELECT id FROM so.codelist WHERE collection = 'wrbversion') AND NEW.wrbversion IS NOT NULL) THEN
        RAISE EXCEPTION 'Table wrbqualifiergrouptype: Invalid value for wrbversion. Must be present in id of wrbreferencesoilgroupvalueso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.foi NOT IN (SELECT id FROM so.codelist WHERE collection = 'FOIType') THEN
        RAISE EXCEPTION 'Table observableproperty: Invalid value for foi. Must be present in id of foiso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF NEW.role NOT IN (SELECT id FROM so.codelist WHERE collection = 'ResponsiblePartyRole') AND NEW.role IS NOT NULL THEN
        RAISE EXCEPTION 'Table relatedparty: Invalid value for role. Must be present in id of responsiblepartyroleso.codelist.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
COMMENT ON TABLE so.codelist IS 'Table to manageso.codelists.';
-- Add Comment Column
COMMENT ON COLUMN so.codelist.id IS 'Uri of theso.codelist value';
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilInvestigationPurposeValue/generalSoilSurvey', 'general soil survey', 'Soil characterisation with unbiased selection of investigation location.', 'SoilInvestigationPurposeValue', null, null, null, null);
...
-- FEATURE soilplot
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/SoilPlotTypeValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilPlotTypeValue/borehole', 'borehole', 'Penetration into the sub-surface with removal of soil/rock material by using, for instance, a hollow tube-shaped tool, in order to carry out profile descriptions, sampling and/or field tests.', 'SoilPlotTypeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilPlotTypeValue/sample', 'sample', 'Exacavation where soil material is removed as a soil sample without doing any soil profile description.', 'SoilPlotTypeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilPlotTypeValue/trialPit', 'trial pit', 'Excavation or other exposition of the soil prepared to carry out profile descriptions, sampling and/or field tests.', 'SoilPlotTypeValue', null, null, null, null);

-- *** INTERNAL *** 
-- WRBRversion
-- FEATURE  soilprofile
-- so.codelist for internal management of Qgis forms based on real URI of WRB Classification

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue', 'WRB 2006', null, 'wrbversion', null, null, null, null);
...
-- FEATURE soilprofile
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/acrisol', 'Acrisols', 'Soil having an argic horizon, CECclay < 50%.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/albeluvisol', 'Albeluvisols', 'Soil having an argic horizon and albeluvic tonguin.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/alisol', 'Alisols', 'Soil having an argic horizon with CECclay >24 and BS < 50%.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/andosol', 'Andosols', 'Soil having an andic or vitric horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/anthrosol', 'Anthrosols', 'Soils profoundly modified through long-term human activities.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/arenosol', 'Arenosols', 'Soil having a coarse texture up to >100 cm depth.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/calcisol', 'Calcisols', 'Soil having a calcic or petrocalcic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/cambisol', 'Cambisols', 'Soil having a cambic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/chernozem', 'Chernozems', 'Soil having a chernic or blackish mollic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/cryosol', 'Cryosols', 'Soil having a cryic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/durisol', 'Durisols', 'Soil having a duric or petroduric horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/ferralsol', 'Ferralsols', 'Soil having a ferralic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/fluvisol', 'Fluvisols', 'Soil having a fluvic materials.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/gleysol', 'Gleysols', 'Soil having a gleyic properties.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/gypsisol', 'Gypsisols', 'Soil having a gypsic or petrogypsic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/histosol', 'Histosols', 'Soil having organic matter >40 cm depth.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/kastanozem', 'Kastanozems', 'Soil having a brownish mollic horizon and secondary CaCO3.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/leptosol', 'Leptosols', 'Shallow soils, <=25 cm deep', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/lixisol', 'Lixisols', 'Soil having an argic horizon and CECclay <24.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/luvisol', 'Luvisols', 'Soil having an argic horizon and CECclay >24.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/nitisol', 'Nitisols', 'Soil having a nitic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/phaeozem', 'Phaeozems', 'Soil having a mollic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/planosol', 'Planosols', 'Soil having reducing condition and pedogenetic abrupt textural change.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/plinthosol', 'Plinthosols', 'Soil having plinthite or petroplinthite.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/podzol', 'Podzols', 'Soil having a spodic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/regosol', 'Regosols', 'Soil without a diagnostic horizon', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/solonchak', 'Solonchaks', 'Soil having a salic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/solonetz', 'Solonetzs', 'Soil having a natric horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/stagnosol', 'Stagnosols', 'Soil having reducing condition.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/technosol', 'Technosols', 'Soil having a human artefacts.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/umbrisol', 'Umbrisols', 'Soil having an umbric horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBReferenceSoilGroupValue/vertisol', 'Vertisols', 'Soil having a vertic horizon.', 'WRBReferenceSoilGroupValue', null, null, null, null);


-- WRBReferenceSoilGroupValue (2014)
-- FEATURE soilprofile
-- so.codelist AGROPRTAL
...
-- so.codelist INSPIRE
-- https://inspire.ec.europa.eu/so.codelist/OtherSoilNameTypeValue
...
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/LayerTypeValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LayerTypeValue/depthInterval', 'depth interval', 'Fixed depth range where soil is described and/or samples are taken.', 'LayerTypeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LayerTypeValue/geogenic', 'geogenic', 'Domain of the soil profile composed of material resulting from the same, non-pedogenic process, e.g. sedimentation, that might display an unconformity to possible over- or underlying adjacent domains.', 'LayerTypeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LayerTypeValue/subSoil', 'subsoil', 'Natural soil material below the topsoil and overlying the unweathered parent material.', 'LayerTypeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LayerTypeValue/topSoil', 'topsoil', 'Upper part of a natural soil that is generally dark coloured and has a higher content of organic matter and nutrients when compared to the (mineral) horizons below excluding the humus layer.', 'LayerTypeValue', null, null, null, null);


-- LithologyValue
-- FEATURE profileelement
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/LithologyValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/acidicIgneousMaterial', 'acidicIgneousMaterial', 'acidicIgneousMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/acidicIgneousRock', 'acidicIgneousRock', 'acidicIgneousRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/alkaliFeldsparRhyolite', 'alkaliFeldsparRhyolite', 'alkaliFeldsparRhyolite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/amphibolite', 'amphibolite', 'amphibolite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/anthropogenicMaterial', 'anthropogenicMaterial', 'anthropogenicMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/ashAndLapilli', 'ashAndLapilli', 'ashAndLapilli', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/ashBrecciaBombOrBlockTephra', 'ashBrecciaBombOrBlockTephra', 'ashBrecciaBombOrBlockTephra', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/basicIgneousMaterial', 'basicIgneousMaterial', 'basicIgneousMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/boulderGravelSizeSediment', 'boulderGravelSizeSediment', 'boulderGravelSizeSediment', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/breccia', 'breccia', 'breccia', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/carbonateMudstone', 'carbonateMudstone', 'carbonateMudstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/carbonateRichMudstone', 'carbonateRichMudstone', 'carbonateRichMudstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/carbonateSedimentaryMaterial', 'carbonateSedimentaryMaterial', 'carbonateSedimentaryMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/carbonateSedimentaryRock', 'carbonateSedimentaryRock', 'carbonateSedimentaryRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/cataclasiteSeries', 'cataclasiteSeries', 'cataclasiteSeries', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/chalk', 'chalk', 'chalk', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/chemicalSedimentaryMaterial', 'chemicalSedimentaryMaterial', 'chemicalSedimentaryMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/chloriteActinoliteEpidoteMetamorphicRock', 'chloriteActinoliteEpidoteMetamorphicRock', 'chloriteActinoliteEpidoteMetamorphicRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/clasticSedimentaryMaterial', 'clasticSedimentaryMaterial', 'clasticSedimentaryMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/crystallineCarbonate', 'crystallineCarbonate', 'crystallineCarbonate', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/dacite', 'dacite', 'dacite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/dolomite', 'dolomite', 'dolomite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/eclogite', 'eclogite', 'eclogite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/foliatedMetamorphicRock', 'foliatedMetamorphicRock', 'foliatedMetamorphicRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/fragmentalIgneousMaterial', 'fragmentalIgneousMaterial', 'fragmentalIgneousMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/framestone', 'framestone', 'framestone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/genericConglomerate', 'genericConglomerate', 'genericConglomerate', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/genericMudstone', 'genericMudstone', 'genericMudstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/genericSandstone', 'genericSandstone', 'genericSandstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/gneiss', 'gneiss', 'gneiss', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/grainstone', 'grainstone', 'grainstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/granite', 'granite', 'granite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/granodiorite', 'granodiorite', 'granodiorite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/granofels', 'granofels', 'granofels', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/granulite', 'granulite', 'granulite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/hornfels', 'hornfels', 'hornfels', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/hybridSediment', 'hybridSediment', 'hybridSediment', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/hybridSedimentaryRock', 'hybridSedimentaryRock', 'hybridSedimentaryRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/igneousMaterial', 'igneousMaterial', 'igneousMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/igneousRock', 'igneousRock', 'igneousRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/impureCarbonateSedimentaryRock', 'impureCarbonateSedimentaryRock', 'impureCarbonateSedimentaryRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/impureDolomite', 'impureDolomite', 'impureDolomite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/impureLimestone', 'impureLimestone', 'impureLimestone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/intermediateCompositionIgneousMaterial', 'intermediateCompositionIgneousMaterial', 'intermediateCompositionIgneousMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/limestone', 'limestone', 'limestone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/marble', 'marble', 'marble', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/materialFormedInSurficialEnvironment', 'materialFormedInSurficialEnvironment', 'materialFormedInSurficialEnvironment', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/metamorphicRock', 'metamorphicRock', 'metamorphicRock', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/micaSchist', 'micaSchist', 'micaSchist', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/migmatite', 'migmatite', 'migmatite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/mineDumpMaterial', 'mineDumpMaterial', 'mineDumpMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/monzogranite', 'monzogranite', 'monzogranite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/mudSizeSediment', 'mudSizeSediment', 'mudSizeSediment', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/naturalUnconsolidatedMaterial', 'naturalUnconsolidatedMaterial', 'naturalUnconsolidatedMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/nonClasticSiliceousSedimentaryMaterial', 'nonClasticSiliceousSedimentaryMaterial', 'nonClasticSiliceousSedimentaryMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/organicBearingMudstone', 'organicBearingMudstone', 'organicBearingMudstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/organicRichSedimentaryMaterial', 'organicRichSedimentaryMaterial', 'organicRichSedimentaryMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/packstone', 'packstone', 'packstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/peat', 'peat', 'peat', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/phyllite', 'phyllite', 'phyllite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/sandSizeSediment', 'sandSizeSediment', 'sandSizeSediment', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/silicateMudstone', 'silicateMudstone', 'silicateMudstone', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/skarn', 'skarn', 'skarn', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/slate', 'slate', 'slate', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/syenogranite', 'syenogranite', 'syenogranite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/tephra', 'tephra', 'tephra', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/tonalite', 'tonalite', 'tonalite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/tuffite', 'tuffite', 'tuffite', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/unconsolidatedMaterial', 'unconsolidatedMaterial', 'unconsolidatedMaterial', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/waste', 'waste', 'waste', 'LithologyValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LithologyValue/carbonateWackestone', 'https://inspire.ec.europa.eu/so.codelist/LithologyValue/carbonateWackestone', 'https://inspire.ec.europa.eu/so.codelist/LithologyValue/carbonateWackestone', 'LithologyValue', null, null, null, null);


-- EventProcessValue
-- FEATURE profileelement
-- so.codelist INSPIRE
--http://inspire.ec.europa.eu/so.codelist/EventProcessValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/bolideImpact', 'bolide impact', 'The impact of an extraterrestrial body on the surface of the earth.', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/cometaryImpact', 'cometary impact', 'the impact of a comet on the surface of the earth', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/meteoriteImpact', 'meteorite impact', 'the impact of a meteorite on the surface of the earth', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/deepWaterOxygenDepletion', 'deep water oxygen depletion', 'Process of removal of oxygen from from the deep part of a body of water.', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/ductileFlow', 'ductile flow', 'deformation without apparent loss of continuity at the scale of observation.', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/fracturing', 'fracturing', 'The formation of a surface of failure resulting from stress', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/geomagneticProcess', 'geomagnetic process', 'Process that results in change in Earth''s magnetic field.', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/magneticFieldReversal', 'magnetic field reversal', 'geomagnetic event', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/humanActivity', 'human activity', 'Processes of human modification of the earth to produce geologic features.', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/excavation', 'excavation', 'removal of material, as in a mining operation', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/grading', 'grading', 'leveling of earth surface by rearrangement of prexisting material', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/mixing', 'mixing', 'Mixing', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/magmaticProcess', 'magmatic process', 'A process involving melted rock (magma).', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/intrusion', 'intrusion', 'The process of emplacement of magma in pre-existing rock', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/melting', 'melting', 'change of state from a solid to a liquid', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/seaLevelChange', 'sea level change', 'Process of mean sea level changing relative to some datum.', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/seaLevelFall', 'sea level fall', 'process of mean sea level falling relative to some datum', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/seaLevelRise', 'sea level rise', 'process of mean sea level rising relative to some datum', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/speciation', 'speciation', 'Process that results inappearance of new species.', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/orogenicProcess', 'orogenic process', 'mountain building process.', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/subduction', 'subduction', 'The process of one lithospheric plate descending beneath another', 'EventProcessValue', null, null, null, null);
...
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/digging', 'digging', 'repeated mixing of loose surface material by digging with a spade or similar tool', 'EventProcessValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventProcessValue/geologicProcess', 'geologic process', 'process that effects the geologic record', 'EventProcessValue', null, null, null, null);


-- EventEnvironmentValue
-- FEATURE profileelement
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/agriculturalAndForestryLandSetting''', 'agricultural and forestry land setting', 'Human influence setting with intensive agricultural activity or forestry land use,  including forest plantations.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/carbonateShelfSetting', 'carbonate shelf setting', 'A type of carbonate platform that is attached to a continental landmass and a region of sedimentation that is analogous to shelf environments for terrigenous clastic deposition. A carbonate shelf may receive some supply of material from the adjacent landmass.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/deltaSlopeSetting', 'delta slope setting', 'Slope setting within the deltaic  system.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/dwellingAreaSetting', 'dwelling area setting', 'Dwelling area setting.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/earthInteriorSetting', 'earth interior setting', 'Geologic environments within the solid Earth.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/earthSurfaceSetting', 'earth surface setting', 'Geologic environments on the surface of the solid Earth.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/extraTerrestrialSetting', 'extra-terrestrial setting', 'Material originated outside of the Earth or its atmosphere.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/fanDeltaSetting', 'fan delta setting', 'A debris-flow or sheetflood-dominated alluvial fan build out into a lake or the sea.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/foreshore', 'foreshore', 'A foreshore is the region between mean high water and mean low water marks of the tides. Depending on the tidal range this may be a vertical distance of anything from a few tens of centimetres to many meters.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/glaciofluvialSetting', 'glaciofluvial setting', 'A setting influenced by glacial meltwater streams. This setting can be sub- en-, supra- and proglacial.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/glaciolacustrineSetting', 'glaciolacustrine setting', 'Ice margin lakes and other lakes related to glaciers. Where meltwater streams enter the lake, sands and gravels are deposited in deltas. At the lake floor, typivally rhythmites (varves) are deposited.Ice margin lakes and other lakes related to glaciers.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/glaciomarineSetting', 'glaciomarine setting', 'A marine environment influenced by glaciers. Dropstone diamictons and dropstone muds are typical deposits in this environment.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/graben', 'graben', 'An elongate trough or basin, bounded on both sides by high-angle normal faults that dip toward one another. It is a structual form that may or may not be geomorphologically expressed as a rift valley.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/halfGraben', 'half-graben', 'A elongate , asymmetric trough or basin bounded on one side by a normal fault.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/humanEnvironmentSetting', 'human environment setting', 'Human environment setting.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/intracratonicSetting', 'intracratonic setting', 'A basin formed within the interior region  of a continent, away from plate boundaries.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/landReclamationSetting', 'land reclamation setting', '''Human influence setting making land capable of more intensive use by changing its general character, as by drainage of excessively wet land, irrigation of arid or semiarid land; or recovery of submerged land from seas, lakes and rivers, restoration after human-induced degradation by removing toxic substances.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/miningAreaSetting', 'mining area setting', 'Human influence setting in which mineral resources are extracted from the ground.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/saltPan', 'salt pan', 'A small, undrained, shallow depression in which water accumulates and evaporates, leaving a salt deposit.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/tectonicallyDefinedSetting', 'tectonically defined setting', 'Setting defined by relationships to tectonic plates on or in the Earth.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/wasteAndMaterialDepositionAreaSetting', 'waste and material deposition area setting', 'Human influence setting in which non-natural or natural materials from elsewhere are deposited.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/wetToSubHumidSetting', 'wet to sub-humid setting', '''A Wet to sub-humid climate is according Thornthwaite''s climate classification system associated with rain forests (wet), forests (humid) and grassland (sub-humid).''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/fastSpreadingCenterSetting', 'fast spreading center setting', 'Spreading center at which the opening rate is greater than 100 mm per year.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/mediumRateSpreadingCenterSetting', 'medium-rate spreading center setting', 'Spreading center at which the opening rate is between 50 and 100 mm per year.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/slowSpreadingCenterSetting', 'slow spreading center setting', 'Spreading center at which the opening rate is less than 50 mm per year.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/dunefieldSetting', 'dunefield setting', '''Extensive deposits on sand in an area where the supply is abundant. As a characteristic, individual dunes somewhat resemble barchans but are highly irregular in shape and crowded; erg areas of the Sahara are an example.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/dustAccumulationSetting', 'dust accumulation setting', 'Setting in which finegrained particles accumulate, e.g. loess deposition.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/sandPlainSetting', 'sand plain setting', 'A sand-covered plain dominated by aeolian processes.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/gibberPlainSetting', 'gibber plain setting', '''A desert plain strewn with wind-abraded pebbles, or gibbers; a gravelly desert.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/marginalMarineSabkhaSetting', 'marginal marine sabkha setting', 'Setting characterized by arid to semi-arid conditions on restricted coastal plains mostly above normal high tide level, with evaporite-saline mineral, tidal-flood, and eolian deposits. Boundaries with intertidal setting and non-tidal terrestrial setting are gradational. (Jackson, 1997, p. 561).', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/playaSetting', 'playa setting', 'The usually dry and nearly level plain that occupies the lowest parts of closed depressions, such as those occurring on intermontane basin floors. Temporary flooding occurs primarily in response to precipitation-runoff events.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/barrierBeachSetting', 'barrier beach setting', '''A narrow, elongate sand or gravel ridge rising slightly above the high-tide level and extending generally parallel with the shore, but separated from it by a lagoon (Shepard, 1954, p.1904), estuary, or marsh; it is extended by longshore transport and is rarely more than several kilometers long.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/barrierLagoonSetting', 'barrier lagoon setting', 'A lagoon that is roughly parallel to the coast and is separated from the open ocean by a strip of land or by a barrier reef. Tidal influence is typically restricted and the lagoon is commonly hypersaline.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lowerBathyalSetting', 'lower bathyal setting', 'The ocean environment at depths between 1000 and 3500 metres.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/middleBathyalSetting', 'middle bathyal setting', 'The ocean environment at water depths between 600 and 1000 metres.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/upperBathyalSetting', 'upper bathyal setting', 'The ocean environment at water depths between 200 and 600 metres.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/backreefSetting', 'backreef setting', '''The landward side of a reef. The term is often used adjectivally to refer to deposits within the restricted lagoon behind a barrier reef, such as the ''back-reef facies'' of lagoonal deposits. In some places, as on a platform-edge reef tract, ''back reef'' refers to the side of the reef away from the open sea, even though no land may be nearby.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/forereefSetting', 'forereef setting', '''The seaward side of a reef; the slope covered with deposits of coarse reef talus.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/reefFlatSetting', 'reef flat setting', 'A stony platform of reef rock, landward of the reef crest at or above the low tide level, occasionally with patches of living coral and associated organisms, and commonly strewn with coral fragments and coral sand.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/basinBogSetting', 'basin bog setting', 'An ombrotrophic or ombrogene peat/bog whose nutrient supply is exclusively from rain water (including snow and atmospheric fallout) therefore making nutrients extremely oligotrophic.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/blanketBog', 'blanket bog', '''Topogeneous bog/peat whose moisture content is largely dependent on surface water. It is relatively rich in plant nutrients, nitrogen, and mineral matter, is mildly acidic to nearly neutral, and contains little or no cellulose; forms in topographic depressions with essential stagnat or non-moving minerotrophic water supply''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/collisionalSetting', 'collisional setting', 'ectonic setting in which two continental crustal plates impact and are sutured together after intervening oceanic crust is entirely consumed at a subduction zone separating the plates. Such collision typically involves major mountain forming events, exemplified by the modern Alpine and Himalayan mountain chains.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/forelandSetting', 'foreland setting', 'The exterior area of an orogenic belt where deformation occurs without significant metamorphism. Generally the foreland is closer to the continental interior than other portions of the orogenic belt are.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/hinterlandTectonicSetting', 'hinterland tectonic setting', '''Tectonic setting in the internal part of an orogenic belt, characterized by plastic deformation of rocks accompanied by significant metamorphism, typically involving crystalline basement rocks. Typically denotes the most structurally thickened part of an orogenic belt, between a magmatic arc or collision zone and a more ''external'' foreland setting.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lowerContinentalCrustalSetting', 'lower continental-crustal setting', 'Continental crustal setting characterized by upper amphibolite to granulite facies metamorphism, in situ melting, residual anhydrous metamorphic rocks, and ductile flow of rock bodies.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/middleContinentalCrustSetting', 'middle continental crust setting', 'Continental crustal setting characterized by greenschist to upper amphibolite facies metamorphism, plutonic igneous rocks, and ductile deformation.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/upperContinentalCrustalSetting', 'upper continental crustal setting', 'Continental crustal setting dominated by non metamorphosed to low greenschist facies metamorphic rocks, and brittle deformation.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/continentalCrustalSetting', 'continental-crustal setting', '''That type of the Earth''s crust which underlies the continents and the continental shelves; it is equivalent to the sial and continental sima and ranges in thickness from about 25 km to more than 70 km under mountain ranges, averaging ~40 km. The density of the continental crust averages ~2.8 g/cm3 and is ~2.7 g.cm3 in the upper layer. The velocities of compressional seismic waves through it average ~6.5 km/s and are less than ~7.0 km/sec.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/oceanicCrustalSetting', 'oceanic-crustal setting', '''That type of the Earth''s crust which underlies the ocean basins. The oceanic crust is 5-10 km thick; it has a density of 2.9 g/cm3, and compressional seismic-wave velocities travelling through it at 4-7.2 km/sec. Setting in crust produced by submarine volcanism at a mid ocean ridge.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/transitionalCrustalSetting', 'transitional-crustal setting', 'Crust formed in the transition zone between continental and oceanic crust, during the history of continental rifting that culminates in the formation of a new ocean.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/deltaDistributaryChannelSetting', 'delta distributary channel setting', 'A divergent stream flowing away from the main stream and not returning to it, as in a delta or on an alluvial plain.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/deltaDistributaryMouthSetting', 'delta distributary mouth setting', 'The mouth of a delta distributary channel where fluvial discharge moves from confined to unconfined flow conditions.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/deltaFrontSetting', 'delta front setting', '''A narrow zone where deposition in deltas is most active, consisting of a continuous sheet of sand, and occurring within the effective depth of wave erosion (10 m or less). It is the zone separating the prodelta from the delta plain, and it may or may not be steep''''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/deltaPlainSetting', 'delta plain setting', '''The level or nearly level surface composing the landward part of a large or compound delta; strictly, an alluvial plain characterized by repeated channel bifurcation and divergence, multiple distributary channels, and interdistributary flood basins.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/estuarineDeltaSetting', 'estuarine delta setting', 'A delta that has filled, or is in the process of filling, an estuary.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/interdistributaryBaySetting', 'interdistributary bay setting', 'A pronounced indentation of the delta front between advancing stream distributaries, occupied by shallow water, and either open to the sea or partly enclosed by minor distributaries.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lacustrineDeltaSetting', 'lacustrine delta setting', 'The low, nearly flat, alluvial tract of land at or near the mouth of a river, commonly forming a triangular or fan-shaped plain of considerable area, crossed by many distributaries of the main river, perhaps extending beyond the general trend of the lake shore, resulting from the accumulation of sediment supplied by the river in such quantities that it is not removed by waves or currents. Most deltas are partly subaerial and partly below water.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/prodeltaSetting', 'prodelta setting', '''The part of a delta that is below the effective depth of wave erosion, lying beyond the delta front, and sloping gently down to the floor of the basin into which the delta is advancing and where clastic river sediment ceases to be a significant part of the basin-floor deposits; it is entirely below the water level.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lowerDeltaPlainSetting', 'lower delta plain setting', 'The part of a delta plain which is penetrated by saline water and is subject to tidal processes.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/upperDeltaPlainSetting', 'upper delta plain setting', 'The part of a delta plain essentially unaffected by basinal processes. They do not differ substantially from alluvial environments except that areas of swamp, marsh and lakes are usually more widespread and channels may bifurcate downstream.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/coastalDuneFieldSetting', 'coastal dune field setting', '''A dune field on low-lying land recently abandoned or built up by the sea; the dunes may ascend a cliff and travel inland.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/contactMetamorphicSetting', 'contact metamorphic setting', 'Metamorphism of country rock at the contact of an igneous body.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/crustalSetting', 'crustal setting', '''The outermost layer or shell of the Earth, defined according to various criteria, including seismic velocity, density and composition; that part of the Earth above the Mohorovicic discontinuity, made up of the sial and the sima.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/highPressureLowTemperatureEarthInteriorSetting', 'high pressure low temperature Earth interior setting', '''High pressure environment characterized by geothermal gradient significantly lower than standard continental geotherm; environment in which blueschist facies metamorphic rocks form. Typically associated with subduction zones.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/hypabyssalSetting', 'hypabyssal setting', '''Igneous environment close to the Earth''s surface, characterized by more rapid cooling than plutonic setting to produce generally fine-grained intrusive igneous rock that is commonly associated with co-magmatic volcanic rocks.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lowPressureHighTemperatureSetting', 'low pressure high temperature setting', 'Setting characterized by temperatures significantly higher that those associated with normal continental geothermal gradient.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/mantleSetting', 'mantle setting', 'The zone of the Earth below the crust and above the core, which is divided into the upper mantle and the lower mantle, with a transition zone separating them.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/regionalMetamorphicSetting', 'regional metamorphic setting', '''Metamorphism not obviously localized along contacts of igneous bodies; includes burial metamorphism and ocean ridge metamorphism.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/ultraHighPressureCrustalSetting', 'ultra high pressure crustal setting', 'Setting characterized by pressures characteristic of upper mantle, but indicated by mineral assemblage in crustal composition rocks.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/anoxicSetting', 'anoxic setting', 'Setting depleted in oxygen, typically subaqueous.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/aridOrSemiAridEnvironmentSetting', 'arid or Semi Arid environment setting', '''Setting characterized by mean annual precipitation of 10 inches (25 cm) or less. (Jackson, 1997, p. 172). Equivalent to SLTT ''Desert setting'', but use ''Arid'' to emphasize climatic nature of setting definition.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/caveSetting', 'cave setting', '''A natural underground open space; it generally has a connection to the surface, is large enough for a person to enter, and extends into darkness. The most common type of cave is formed in limestone by dissolution.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/deltaicSystemSetting', 'deltaic system setting', 'Environments at the mouth of a river or stream that enters a standing body of water (ocean or lake). The delta forms a triangular or fan-shaped plain of considerable area. Subaerial parts of the delta are crossed by many distributaries of the main river,', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/glacierRelatedSetting', 'glacier related setting', 'Earth surface setting with geography defined by spatial relationship to glaciers (e.g. on top of a glacier, next to a glacier, in front of a glacier...). Processes related to moving ice dominate sediment transport and deposition and landform development.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/hillslopeSetting', 'hillslope setting', 'Earth surface setting characterized by surface slope angles high enough that gravity alone becomes a significant factor in geomorphic development, as well as base-of-slope areas influenced by hillslope processes. Hillslope activities include creep, sliding, slumping, falling, and other downslope movements caused by slope collapse induced by gravitational influence on earth materials. May be subaerial or subaqueous.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/humidTemperateClimaticSetting', 'humid temperate climatic setting', 'Setting with seasonal climate having hot to cold or humid to arid seasons.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/humidTropicalClimaticSetting', 'humid tropical climatic setting', 'Setting with hot, humid climate influenced by equatorial air masses, no winter season.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/polarClimaticSetting', 'polar climatic setting', 'Setting with climate dominated by temperatures below the freezing temperature of water. Includes polar deserts because precipitation is generally scant at high latitude. Climatically controlled by arctic air masses, cold dry environment with short summer.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/shorelineSetting', 'shoreline setting', 'Geologic settings characterized by location adjacent to the ocean or a lake. A zone of indefinite width (may be many kilometers), bordering a body of water that extends from the water line inland to the first major change in landform features. Includes settings that may be subaerial, intermittently subaqueous, or shallow subaqueous, but are intrinsically associated with the interface between land areas and water bodies.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/subaerialSetting', 'subaerial setting', 'Setting at the interface between the solid earth and the atmosphere, includes some shallow subaqueous settings in river channels and playas. Characterized by conditions and processes, such as erosion, that exist or operate in the open air on or immediately adjacent to the land surface.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/subaqueousSetting', 'subaqueous setting', 'Setting situated in or under permanent, standing water. Used for marine and lacustrine settings, but not for fluvial settings.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/terrestrialSetting', 'terrestrial setting', 'Setting characterized by absence of direct marine influence. Most of the subaerial settings are also terrestrial, but lacustrine settings, while terrestrial, are not subaerial, so the subaerial settings are not included as subcategories.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/wetlandSetting', 'wetland setting', 'Setting characterized by gentle surface slope, and at least intermittent presence of standing water, which may be fresh, brackish, or saline. Wetland may be terrestrial setting or shoreline setting.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/estuarineLagoonSetting', 'estuarine lagoon setting', '''A lagoon produced by the temporary sealing of a river estuary by a storm barrier. Such lagoons are usually seasonal and exist until the river breaches the barrier; they occur in regions of low or spasmodic rainfall.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/continentalRiftSetting', 'continental rift setting', 'Extended terrane in a zone of continental breakup, may include incipient oceanic crust. Examples include Red Sea, East Africa Rift, Salton Trough.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/englacialSetting', 'englacial setting', '''Contained, embedded, or carried within the body of a glacier or ice sheet; said of meltwater streams, till, drift, moraine.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/glacialOutwashPlainSetting', 'glacial outwash plain setting', '''A broad, gently sloping sheet of outwash deposited by meltwater streams flowing in front of or beyond a glacier, and formed by coalescing outwahs fans; the surface of a broad body of outwash.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/glacierLateralSetting''', 'glacier lateral setting', 'Settings adjacent to edges of confined glacier.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/proglacialSetting', 'proglacial setting', '''Immediately in front of or just beyond the outer limits of a glacier or ice sheet, generally at or near its lower end; said of lakes, streams, deposits, and other features produced by or derived from the glacier ice.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/subglacialSetting', 'subglacial setting', '''Formed or accumulated in or by the bottom parts of a glacier or ice sheet; said of meltwater streams, till, moraine, etc.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/supraglacialSetting', 'supraglacial setting', '''''Carried upon, deposited from, or pertaining to the top surface of a glacier or ice sheet; said of meltwater streams, till, drift, etc. '' (Jackson, 1997, p. 639). Dreimanis (1988, p. 39) recommendation that ''supraglacial'' supersede ''superglacial'' is followed.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/inactiveSpreadingCenterSetting', 'inactive spreading center setting', 'Setting on oceanic crust formed at a spreading center that has been abandoned.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/seamountSetting', 'seamount setting', 'Setting that consists of a conical mountain on the ocean floor (guyot). Typically characterized by active volcanism, pelagic sedimentation. If the mountain is high enough to reach the photic zone, carbonate production may result in reef building to produce a carbonate platform or atoll setting.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/algalFlatSetting', 'algal flat setting', '''Modern ''algal flats are found on rock or mud in areas flooded only by the highest tides and are often subject to high evaporation rates. Algal flats survive only when an area is salty enough to eliminate snails and other herbivorous animals that eat algae, yet is not so salty that the algae cannot survive. The most common species of algae found on algal flats are blue-green algae of the genera Scytonema and Schizothrix. These algae can tolerate the daily extremes in temperature and oxygen that typify conditions on the flats. Other plants sometimes found on algal flats include one-celled green algae, flagellates, diatoms, bacteria, and isolated scrubby red and black mangroves, as well as patches of saltwort. Animals include false cerith, cerion snails, fiddler crabs, and great land crabs. Flats with well developed algal mats are restricted for the most part to the Keys, with Sugarloaf and Crane Keys offering prime examples of algal flat habitat.'' (Audubon, 1991)''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/mudFlatSetting', 'mud flat setting', 'A relatively level area of fine grained material (e.g. silt) along a shore (as in a sheltered estuary or chenier-plain) or around an island, alternately covered and uncovered by the tide or covered by shallow water, and barren of vegetation. Includes most tidal flats, but lacks denotation of tidal influence.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lowerMantleSetting', 'lower mantle setting', 'That part of the mantle that lies below a depth of about 660 km. With increasing depth, density increases from ~4.4 g/cm3 to ~5.6 g/cm3, and velocity of compressional seismic waves increases from ~10.7 km/s to ~13.7 km/s (Dziewonski and Anderson, 1981).', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/upperMantleSetting', 'upper mantle setting', 'That part of the mantle which lies above a depth of about 660 km and has a density of 3.4 g/cm3 to 4.0 g/cm3 with increasing depth. Similarly, P-wave velocity increases from about 8 to 11 km/sec with depth and S wave velocity increases from about 4.5 to 6 km/sec with depth. It is presumed to be peridotitic in composition. It includes the subcrustal lithosphere the asthenosphere and the transition zone.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/aboveCarbonateCompensationDepthSetting', 'above carbonate compensation depth setting', 'Marine environment in which carbonate sediment does not dissolve before reaching the sea floor and can accumulate.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/abyssalSetting', 'abyssal setting', 'The ocean environment at water depths between 3,500 and 6,000 metres.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/basinPlainSetting', 'basin plain setting', '''Near flat areas of ocean floor, slope less than 1:1000; generally receive only distal turbidite and pelagic sediments.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/bathyalSetting', 'bathyal setting', 'The ocean environment at water depths between 200 and 3500 metres.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/belowCarbonateCompensationDepthSetting', 'below carbonate compensation depth setting', 'Marine environment in which water is deep enough that carbonate sediment goes into solution before it can accumulate on the sea floor.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/biologicalReefSetting', 'biological reef setting', '''A ridgelike or moundlike structure, layered or massive, built by sedentary calcareous organisms, esp. corals, and consisting mostly of their remains; it is wave-resistant and stands topographically above the surrounding contemporaneously deposited sediment.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/continentalBorderlandSetting', 'continental borderland setting', 'An area of the continental margin between the shoreline and the continental slope that is topographically more complex than the continental shelf. It is characterized by ridges and basins, some of which are below the depth of the continental shelf. An example is the southern California continental borderland (Jackson, 1997, p. 138).', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/continentalShelfSetting', 'continental shelf setting', '''That part of the ocean floor that is between the shoreline and the continental slope (or, when there is no noticeable continental slope, a depth of 200 m). It is characterized by its gentle slope of 0.1 degree (Jackson, 1997, p. 138). Continental shelves have a classic shoreline-shelf-slope profile termed ''clinoform''.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/deepSeaTrenchSetting', 'deep sea trench setting', 'Deep ocean basin with steep (average 10 degrees) slope toward land, more gentle slope (average 5 degrees) towards the sea, and abundant seismic activity on landward side of trench. Does not denote water depth, but may be very deep.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/epicontinentalMarineSetting', 'epicontinental marine setting', 'Marine setting situated within the interior of the continent, rather than at the edge of a continent.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/hadalSetting', 'hadal setting', 'The deepest oceanic environment, i.e., over 6,000 m in depth. Always in deep sea trench.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/marineCarbonatePlatformSetting', 'marine carbonate platform setting', 'A shallow submerged plateau separated from continental landmasses, on which high biological carbonate production rates produce enough sediment to maintain the platform surface near sea level. Grades into atoll as area becomes smaller and ringing coral reefs become more prominent part of the setting.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/neriticSetting', 'neritic setting', 'The ocean environment at depths between low-tide level and 200 metres, or between low-tide level and approximately the edge of the continental shelf.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/oceanHighlandSetting', 'ocean highland setting', 'Broad category for subaqueous marine settings characterized by significant relief above adjacent sea floor.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/slopeRiseSetting', 'slope-rise setting', 'The part of a subaqueous basin that is between a bordering shelf setting, which separate the basin from an adjacent landmass, and a very low-relief basin plain setting.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/submarineFanSetting', 'submarine fan setting', 'Large fan-shaped cones of sediment on the ocean floor, generally associated with submarine canyons that provide sediment supply to build the fan.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/innerNeriticSetting', 'inner neritic setting', 'The ocean environment at depths between low tide level and 30 metres.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/middleNeriticSetting', 'middle neritic setting', 'The ocean environment at depths between 30 and 100 metres.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/outerNeriticSetting', 'outer neritic setting', 'The ocean environment at depths between 100 meters and approximately the edge of the continental shelf or between 100 and 200 meters.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/midOceanRidgeSetting', 'mid ocean ridge setting', 'Ocean highland associated with a divergent continental margin (spreading center). Setting is characterized by active volcanism, locally steep relief, hydrothermal activity, and pelagic sedimentation.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/oceanicPlateauSetting', 'oceanic plateau setting', 'Region of elevated ocean crust that commonly rises to within 2-3 km of the surface above an abyssal sea floor that lies several km deeper. Climate and water depths are such that a marine carbonate platform does not develop.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lowerOceanicCrustalSetting', 'lower oceanic-crustal setting', 'Setting characterized by dominantly intrusive mafic rocks, with sheeted dike complexes in upper part and gabbroic to ultramafic intrusive or metamorphic rocks in lower part.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/upperOceanicCrustalSetting', 'upper oceanic crustal setting', 'Oceanic crustal setting dominated by extrusive rocks, abyssal oceanic sediment, with increasing mafic intrusive rock in lower part.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/alluvialFanSetting', 'alluvial fan setting', '''A low, outspread, relatively flat to gently sloping mass of loose rock material, shaped like an open fan or a segment of a cone, deposited by a stream (esp. in a semiarid region) at the place where it issues from a narrow mountain valley upon a plain or broad valley, or where a tributary stream is near or at its junction with the main stream, or wherever a constriction in a valley abruptly ceases or the gradient of the stream suddenly decreases; it is steepest near the mouth of the valley where its apex points upstream, and it slopes gently and convexly outward with gradually decreasing gradient.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/alluvialPlainSetting', 'alluvial plain setting', '''An assemblage landforms produced by alluvial and fluvial processes (braided streams, terraces, etc.,) that form low gradient, regional ramps along the flanks of mountains and extend great distances from their sources (e.g., High Plains of North America). (NRCS GLOSSARY OF LANDFORM AND GEOLOGIC TERMS). A level or gently sloping tract or a slightly undulating land surface produced by extensive deposition of alluvium... Synonym-- wash plain;...river plain; aggraded valley plain;... (Jackson, 1997, p. 17). May include one or more River plain systems.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/pedimentSetting', 'pediment setting', '''A gently sloping erosional surface developed at the foot of a receding hill or mountain slope. The surface may be essentially bare, exposing earth material that extends beneath adjacent uplands; or it may be thinly mantled with alluvium and colluvium, ultimately in transit from upland front to basin or valley lowland. In hill-foot slope terrain the mantle is designated ''pedisediment.'' The term has been used in several geomorphic contexts: Pediments may be classed with respect to (a) landscape positions, for example, intermontane-basin piedmont or valley-border footslope surfaces (respectively, apron and terrace pediments (Cooke and Warren, 1973)); (b) type of material eroded, bedrock or regolith; or (c) combinations of the above. compare - Piedmont slope.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/activeContinentalMarginSetting', 'active continental margin setting', 'Plate margin setting on continental crust.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/activeSpreadingCenterSetting', 'active spreading center setting', 'Divergent plate margin at which new oceanic crust is being formed.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/forearcSetting', 'forearc setting', 'Tectonic setting between a subduction-related trench and a volcanic arc.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/subductionZoneSetting', 'subduction zone setting', 'Tectonic setting at which a tectonic plate, usually oceanic, is moving down into the mantle beneath another overriding plate.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/transformPlateBoundarySetting', 'transform plate boundary setting', 'Plate boundary at which the adjacent plates are moving laterally relative to each other.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/volcanicArcSetting', 'volcanic arc setting', 'A generally curvillinear belt of volcanoes above a subduction zone.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/glacierTerminusSetting', 'glacier terminus setting', 'Region of sediment deposition at the glacier terminus due to melting of glacier ice, melt-out, ablation and flow till setting.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/braidedRiverChannelSetting', 'braided river channel setting', 'A stream that divides into or follows an interlacing or tangled network of several small branching and reuniting shallow channels separated from each other by ephemeral branch islands or channel bars, resembling in plan the strands of a complex braid. Such a stream is generally believed to indicate an inability to carry all of its load, such as an overloaded and aggrading stream flowing in a wide channel on a floodplain.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/meanderingRiverChannelSetting', 'meandering river channel setting', 'Produced by a mature stream swinging from side to side as it flows across its floodplain or shifts its course laterally toward the convex side of an original curve.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/abandonedRiverChannelSetting', 'abandoned river channel setting', 'A drainage channel along which runoff no longer occurs, as on an alluvial fan.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/cutoffMeanderSetting', 'cutoff meander setting', 'The abandoned, bow- or horseshoe-shaped channel of a former meander, left when the stream formed a cutoff across a narrow meander neck. Note that these are typically lakes, thus also lacustrine.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/floodplainSetting', 'floodplain setting', 'The surface or strip of relatively smooth land adjacent to a river channel, constructed by the present river in its existing regimen and covered with water when the river overflows its banks. It is built of alluvium carried by the river during floods and deposited in the sluggish water beyond the influence of the swiftest current. A river has one floodplain and may have one or more terraces representing abandoned floodplains.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/riverChannelSetting', 'river channel setting', '''The bed where a natural body of surface water flows or may flow; a natural passageway or depression of perceptible extent containing continuously or periodically flowing water, or forming a connecting link between two bodies of water; a watercourse.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/springSetting', 'spring setting', 'Setting characterized by a place where groundwater flows naturally from a rock or the soil onto the land surface or into a water body.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/barrierIslandCoastlineSetting', 'barrier island coastline setting', 'Setting meant to include all the various geographic elements typically associated with a barrier island coastline, including the barrier islands, and geomorphic/geographic elements that are linked by processes associated with the presence of the island (e.g. wash over fans, inlet channel, back barrier lagoon).''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/beachSetting', 'beach setting', '''The unconsolidated material at the shoreline that covers a gently sloping zone, typically with a concave profile, extending landward from the low-water line to the place where there is a definite change in material or physiographic form (such as a cliff), or to the line of permanent vegetation (usually the effective limit of the highest storm waves); at the shore of a body of water, formed and washed by waves or tides, usually covered by sand or gravel, and lacking a bare rocky surface.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/carbonateDominatedShorelineSetting', 'carbonate dominated shoreline setting', 'A shoreline setting in which terrigenous input is minor compared to local carbonate sediment production. Constructional biogenic activity is an important element in geomorphic development.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/coastalPlainSetting', 'coastal plain setting', 'A low relief plain bordering a water body extending inland to the nearest elevated land, sloping very gently towards the water body. Distinguished from alluvial plain by presence of relict shoreline-related deposits or morphology.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/estuarySetting', 'estuary setting', 'Environments at the seaward end or the widened funnel-shaped tidal mouth of a river valley where fresh water comes into contact with seawater and where tidal effects are evident (adapted from Glossary of Geology, Jackson, 1997, p. 217).', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lagoonalSetting', 'lagoonal setting', '''A shallow stretch of salt or brackish water, partly or completely separated from a sea or lake by an offshore reef, barrier island, sand or spit (Jackson, 1997). Water is shallow, tidal and wave-produced effects on sediments; strong light reaches sediment.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lowEnergyShorelineSetting', 'low energy shoreline setting', 'Settings characterized by very low surface slope and proximity to shoreline. Generally within peritidal setting, but characterized by low surface gradients and generally low-energy sedimentary processes.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/rockyCoastSetting', 'rocky coast setting', 'Shoreline with significant relief and abundant rock outcrop.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/strandplainSetting', 'strandplain setting', 'A prograded shore built seaward by waves and currents, and continuous for some distance along the coast. It is characterized by subparallel beach ridges and swales, in places with associated dunes.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/supratidalSetting', 'supratidal setting', 'Pertaining to the shore area marginal to the littoral zone, just above high-tide level.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/tidalSetting', 'tidal setting', 'Setting subject to tidal processes.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/aeolianProcessSetting', 'aeolian process setting', 'Sedimentary setting in which wind is the dominant process producing, transporting, and depositing sediment. Typically has low-relief plain or piedmont slope physiography.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/piedmontSlopeSystemSetting', 'piedmont slope system setting', '''Location on gentle slope at the foot of a mountain; generally used in terms of intermontane-basin terrain. Main components include: (a) An erosional surface on bedrock adjacent to the receding mountain front (pediment, rock pediment); (b) A constructional surface comprising individual alluvial fans and interfan valleys, also near the mountain front; and (c) A distal complex of coalescent fans (bajada), and alluvial slopes without fan form. Piedmont slopes grade to basin-floor depressions with alluvial and temporary lake plains or to surfaces associated with through drainage.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/intertidalSetting', 'intertidal setting', '''Pertaining to the benthic ocean environment or depth zone between high water and low water; also, pertaining to the organisms of that environment.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/marineSetting', 'marine setting', 'Setting characterized by location under the surface of the sea.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/tidalChannelSetting', 'tidal channel setting', 'A major channel followed by the tidal currents, extending from offshore into a tidal marsh or a tidal flat.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/tidalMarshSetting', 'tidal marsh setting', '''A marsh bordering a coast (as in a shallow lagoon or sheltered bay), formed of mud and of the resistant mat of roots of salt-tolerant plants, and regularly inundated during high tides; a marshy tidal flat.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/backArcSetting', 'back arc setting', 'Tectonic setting adjacent to a volcanic arc formed above a subduction zone. The back arc setting is on the opposite side of the volcanic arc from the trench at which oceanic crust is consumed in a subduction zone. Back arc setting includes terrane that is affected by plate margin and arc-related processes.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/extendedTerraneSetting', 'extended terrane setting', 'Tectonic setting characterized by extension of the upper crust, manifested by formation of rift valleys or basin and range physiography, with arrays of low to high angle normal faults. Modern examples include the North Sea, East Africa, and the Basin and Range of the North American Cordillera. Typically applied in continental crustal settings.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/hotSpotSetting', 'hot spot setting', 'Setting in a zone of high heat flow from the mantle. Typically identified in intraplate settings, but hot spot may also interact with active plate margins (Iceland...). Includes surface manifestations like volcanic center, but also includes crust and mantle manifestations as well.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/intraplateTectonicSetting', 'intraplate tectonic setting', 'Tectonically stable setting far from any active plate margins.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/passiveContinentalMarginSetting', 'passive continental margin setting', 'Boundary of continental crust into oceanic crust of an oceanic basin that is not a subduction zone or transform fault system. Generally is rifted margin formed when ocean basin was initially formed.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/plateMarginSetting', 'plate margin setting', 'Tectonic setting at the boundary between two tectonic plates.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/plateSpreadingCenterSetting', 'plate spreading center setting', 'Tectonic setting where new oceanic crust is being or has been formed at a divergent plate boundary. Includes active and inactive spreading centers.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/bogSetting', 'bog setting', 'Waterlogged, spongy ground, consisting primarily of mosses, containing acidic, decaying vegetation that may develop into peat.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/lacustrineSetting', 'lacustrine setting', 'Setting associated with a lake. Always overlaps with terrestrial, may overlap with subaerial, subaqueous, or shoreline.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/riverPlainSystemSetting', 'river plain system setting', '''Geologic setting dominated by a river system; river plains may occur in any climatic setting. Includes active channels, abandoned channels, levees, oxbow lakes, flood plain. May be part of an alluvial plain that includes terraces composed of abandoned river plain deposits.''', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/tidalFlatSetting', 'tidal flat setting', 'An extensive, nearly horizontal, barren tract of land that is alternately covered and uncovered by the tide, and consisting of unconsolidated sediment (mostly mud and sand). It may form the top surface of a deltaic deposit.', 'EventEnvironmentValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/EventEnvironmentValue/swampOrMarshSetting', 'swamp or marsh setting', 'A water-saturated, periodically wet or continually flooded area with the surface not deeply submerged, essentially without the formation of peat. Marshes are characterized by sedges, cattails, rushes, or other aquatic and grasslike vegetation. Swamps are characterized by tree and brush vegetation.', 'EventEnvironmentValue', null, null, null, null);


-- LayerGenesisProcessStateValue
-- FEATURE profileelement
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/LayerGenesisProcessStateValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LayerGenesisProcessStateValue/ongoing', 'on-going', 'The process has started in the past and is still active.', 'LayerGenesisProcessStateValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/LayerGenesisProcessStateValue/terminated', 'terminated', 'The process is no longer active.', 'LayerGenesisProcessStateValue', null, null, null, null);


-- FAOHorizonMaster
-- FEATURE faohorizonnotationtype
-- so.codelist INSPIRE
-- https://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue

INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/B', 'B', 'B horizons', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/O', 'O', 'O horizons or layers', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/C', 'C', 'C horizons or layers', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/E', 'E', 'E horizons', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/I', 'I', 'I layers', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/W', 'W', 'W layers', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/H', 'H', 'H horizons or layers', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/A', 'A', 'A horizons', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/R', 'R', 'R layers', 'FAOHorizonMasterValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonMasterValue/L', 'L', 'L layers', 'FAOHorizonMasterValue', null, null, null, null);


-- FAOHorizonSubordinate
-- FEATURE faohorizonnotationtype
-- so.codelist INSPIRE
-- https://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue

INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/a', 'a', 'Highly decomposed organic material', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/b', 'b', 'Buried genetic horizon', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/c', 'c', 'Concretions or nodules', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/c-L', 'c-L', 'Coprogenous earth', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/f', 'f', 'Frozen soil', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/d-L', 'd-L', 'Diatomaceous earth', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/e', 'e', 'Moderately decomposed organic material', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/g', 'g', 'Stagnic conditions', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/d', 'd', 'Dense layer (physically root restrictive)', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/i-HO', 'i-HO', 'Slightly decomposed organic material;"Slightly decomposed organic material: In organic soils and used in combination with H or O horizons, it indicates the state of decomposition of the organic material', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/r', 'r', 'Strong reduction', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/u', 'u', 'Urban and other human-made materials', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/x', 'x', 'Fragipan characteristics', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/k', 'k', 'Accumulation of pedogenetic carbonates', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/l', 'l', 'Capillary fringe mottling (gleying)', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/z', 'z', 'Pedogenetic accumulation of salts more soluble than gypsum', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/@', '@', 'Evidence of cryoturbation', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/j', 'j', 'Jarosite accumulation', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/v', 'v', 'Occurrence of plinthite', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/t', 't', 'Illuvial accumulation of silicate clay', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/m-L', 'm-L', 'Marl', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/n', 'n', 'Pedogenetic accumulation of exchangeable sodium', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/h', 'h', 'Accumulation of organic matter', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/w', 'w', 'Development of colour or structure', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/m', 'm', 'Strong cementation or induration (pedogenetic, massive)', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/q', 'q', 'Accumulation of pedogenetic silica', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/o', 'o', 'Residual accumulation of sesquioxides (pedogenetic)', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/p', 'p', 'Ploughing or other human disturbance', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/s', 's', 'Illuvial accumulation of sesquioxides', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/i', 'i', 'Slickensides', 'FAOHorizonSubordinateValue', null, null, null, null);
INSERT INTO so.codelist (ID, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOHorizonSubordinateValue/y', 'y', 'Pedogenetic accumulation of gypsum', 'FAOHorizonSubordinateValue', null, null, null, null);


-- FAOPrime
-- FEATURE faohorizonnotationtype
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/FAOPrimeValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOPrimeValue/0','0','No Prime applies to this layer or horizon', 'FAOPrimeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOPrimeValue/1','1','One Prime applies to this layer or horizon', 'FAOPrimeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOPrimeValue/2','2','Two Primes apply to this layer or horizon', 'FAOPrimeValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/FAOPrimeValue/3','3','Three Primes apply to this layer or horizon', 'FAOPrimeValue', null, null, null, null);


-- *** INTERNAL *** so.codelist for managing forms 
-- OtherHorizonNotationType
-- FEATURE otherhorizonnotationtype
-- so.codelist for internal management of Qgis forms
...
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/WRBQualifierPlaceValue

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierPlaceValue/suffix', 'suffix', 'Suffix', 'WRBQualifierPlaceValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierPlaceValue/prefix', 'prefix', 'Prefix', 'WRBQualifierPlaceValue', null, null, null, null);


-- WRBQualifierValue (2006)
-- FEATURE wrbqualifiergrouptype
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue


INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Nudiargic', 'Nudiargic', 'Having an argic horizon starting at the mineral soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Ortsteinic', 'Ortsteinic', 'Having a cemented spodic horizon (ortstein) (in Podzols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Aric', 'Aric', 'Having only remnants of diagnostic horizons - disturbed by deep ploughing', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Acric', 'Acric', 'Having an argic horizon that has a CEC (by 1 M NH 4 OAc) of less than 24 cmol c kg -1clay in some part to a maximum depth of 50 cm below its upper limit, either starting within100 cm of the soil surface or within 200 cm of the soil surface if the argic horizon is overlain byloamy sand or coarser textures throughout, and having a base saturation (by 1 M NH 4 OAc) ofless than 50 percent in the major part between 50 and 100 cm from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Alcalic', 'Alcalic', 'Having a pH (1:1 in water) of 8.5 or more throughout within 50 cm of the soilsurface or to continuous rock or a cemented or indurated layer, whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Arzic', 'Arzic', 'Having sulphate-rich groundwater in some layer within 50 cm of the soil surfaceduring some time in most years and containing 15 percent or more gypsum averaged over adepth of 100 cm from the soil surface or to continuous rock or a cemented or indurated layer,whichever is shallower (in Gypsisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Irragric', 'Irragric', 'Having an irragric horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Rheic', 'Rheic', 'Having a histic horizon saturated predominantly with groundwater or flowingsurface water starting within 40 cm of the soil surface (in Histosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Arenic', 'Arenic', 'Having a texture of loamy fine sand or coarser in a layer, 30 cm or more thick,within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Anthric', 'Anthric', 'Having an anthric horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Aluandic', 'Aluandic', 'Having one or more layers, cumulatively 15 cm or more thick, with andicproperties and an acid oxalate (pH 3) extractable silica content of less than 0.6 percent, and anAl py54 /Al ox55 of 0.5 or more, within 100 cm of the soil surface (in Andosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Aridic', 'Aridic', 'Having aridic properties without a takyric or yermic horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Sombric', 'Sombric', 'Having a sombric horizon starting within 150 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Calcaric', 'Calcaric', 'Having calcaric material between 20 and 50 cm from the soil surface or between20 cm and continuous rock or a cemented or indurated layer, whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Alumic', 'Alumic', 'Having an Al saturation (effective) of 50 percent or more in some layer between50 and 100 cm from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Anthraquic', 'Anthraquic', 'Having an anthraquic horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Solodic', 'Solodic', 'Having a layer, 15 cm or more thick within 100 cm of the soil surface, with thecolumnar or prismatic structure of the natric horizon, but lacking its sodium saturationrequirements', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Acroxic', 'Acroxic', 'Having less than 2 cmol c kg -1 fine earth exchangeable bases plus 1 M KClexchangeable Al 3+ in one or more layers with a combined thickness of 30 cm or more within100 cm of the soil surface (in Andosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Andic', 'Andic', 'Having within 100 cm of the soil surface one or more layers with andic or vitricproperties with a combined thickness of 30 cm or more (in Cambisols 15 cm or more), of which15 cm or more (in Cambisols 7.5 cm or more) have andicproperties', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Calcic', 'Calcic', 'Having a calcic horizon or concentrations of secondary carbonates starting within100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Alic', 'Alic', 'Having an argic horizon that has a CEC (by 1 M NH 4 OAc) of 24 cmol c kg -1 clay ormore throughout or to a depth of 50 cm below its upper limit, whichever is shallower, eitherstarting within 100 cm of the soil surface or within 200 cm of the soil surface if the argichorizon is overlain by loamy sand or coarser textures throughout, and having a base saturation(by 1 M NH 4 OAc) of less than 50 percent in the major part between 50 and 100 cm from the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Brunic', 'Brunic', 'Having a layer, 15 cm or more thick, which meets criteria 2-4 of the cambichorizon but fails criterion 1 and does not form part of an albic horizon, starting within 50 cm ofthe soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Endoduric', 'Endoduric', 'Having a duric horizon starting between 50 and 100 cm from the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Carbic', 'Carbic', 'Having a spodic horizon that does not turn redder on ignition throughout (inPodzols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Eutric', 'Eutric', 'Having a base saturation (by 1 M NH 4 OAc) of 50 percent or more in the major partbetween 20 and 100 cm from the soil surface or between 20 cm and continuous rock or acemented or indurated layer, or, in a layer, 5 cm or more thick, directly abovecontinuous rock, if the continuous rock starts within 25 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Chloridic', 'Chloridic', 'Having a salic horizon with a soil solution (1:1 in water) with [Cl - ] >> [SO 42- ] >[HCO 3- ] (in Solonchaks only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Clayic', 'Clayic', 'Having a texture of clay in a layer, 30 cm or more thick, within 100 cm of the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Chromic', 'Chromic', 'Having within 150 cm of the soil surface a subsurface layer, 30 cm or more thick,that has a Munsell hue redder than 7.5 YR or that has both, a hue of 7.5 YR and a chroma,moist, of more than 4', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Densic', 'Densic', 'Having natural or artificial compaction within 50 cm of the soil surface to theextent that roots cannot penetrate', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Glossalbic', 'Glossalbic', 'Showing tonguing of an albic into an argic or natric horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Duric', 'Duric', 'Having a duric horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Colluvic', 'Colluvic', 'Having colluvic material, 20 cm or more thick, created by human-induced lateralmovement', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Carbonatic', 'Carbonatic', 'Having a salic horizon with a soil solution (1:1 in water) with a pH of 8.5 ormore and [HCO 3- ] > [SO 42- ] >> [Cl - ] (in Solonchaks only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Dystric', 'Dystric', 'Having a base saturation (by 1 M NH 4 OAc) of less than 50 percent in the majorpart between 20 and 100 cm from the soil surface or between 20 cm and continuous rock or acemented or indurated layer, or, in a layer, 5 cm or more thick, directly abovecontinuous rock, if the continuous rock starts within 25 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Thaptandic', 'Thaptandic', 'Having within 100 cm of the soil surface one or more buried layerswith andic or vitric properties with a combined thickness of 30 cm or more (in Cambisols15 cm or more), of which 15 cm or more (in Cambisols 7.5 cm or more) have andic properties', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Pisocalcic', 'Pisocalcic', 'Having only concentrations of secondary carbonates starting within 100cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Cambic', 'Cambic', 'Having a cambic horizon, which does not form part of an albic horizon, startingwithin 50 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Cryic', 'Cryic', 'Having a cryic horizon starting within 100 cm of the soil surface or having a cryichorizon starting within 200 cm of the soil surface with evidence of cryoturbation in some layerwithin 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Eutrosilic', 'Eutrosilic', 'Having one or more layers, cumulatively 30 cm or more thick, with andicproperties and a sum of exchangeable bases of 15 cmol c kg -1 fine earth or more within 100 cmof the surface (in Andosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Drainic', 'Drainic', 'Having a histic horizon that is drained artificially starting within 40 cm of the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Ferralic', 'Ferralic', 'Having a ferralic horizon starting within 200 cm of the soil surface (in Anthrosolsonly), or having ferralic properties in at least some layer starting within 100 cm of the soilsurface (in other soils)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Cutanic', 'Cutanic', 'Having clay coatings in some parts of an argic horizon either starting within100 cm of the soil surface or within 200 cm of the soil surface if the argic horizon is overlain byloamy sand or coarser textures throughout', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Ferric', 'Ferric', 'Having a ferric horizon starting within 100 cm of the soil surface.', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Gelistagnic', 'Gelistagnic', 'Having temporary water saturation at the soil surface caused by a frozensubsoil', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Fibric', 'Fibric', 'Having, after rubbing, two-thirds or more (by volume) of the organic materialconsisting of recognizable plant tissue within 100 cm of the soil surface (in Histosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Greyic', 'Greyic', 'Having Munsell colours with a chroma of 3 or less when moist, a value of 3 or lesswhen moist and 5 or less when dry and uncoated silt and sand grains on structural faces within5 cm of the mineral soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Fluvic', 'Fluvic', 'Having fluvic material in a layer, 25 cm or more thick, within 100 cm of the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Glossic', 'Glossic', 'Showing tonguing of a mollic or umbric horizon into an underlying layer', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Gibbsic', 'Gibbsic', 'Having a layer, 30 cm or more thick, containing 25 percent or more gibbsite in thefine earth fraction starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Fractiplinthic', 'Fractiplinthic', 'Having a petroplinthic horizon consisting of fractured or broken clods withan average horizontal length of less than 10 cm, starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Fractipetric', 'Fractipetric', 'Having a strongly cemented or indurated horizon consisting of fractured orbroken clods with an average horizontal length of less than 10 cm, starting within 100 cm of thesoil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Gelic', 'Gelic', 'Having a layer with a soil temperature of 0 ºC or less for two or more consecutiveyears starting within 200 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Gleyic', 'Gleyic', 'Having within 100 cm of the mineral soil surface a layer, 25 cm or more thick, thathas reducing conditions in some parts and  a gleyic colour pattern throughout', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Floatic', 'Floatic', 'Having organic material floating on water (in Histosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Folic', 'Folic', 'Having a folic horizon starting within 40 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Gypsic', 'Gypsic', 'Having a gypsic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Geric', 'Geric', 'Having geric properties in some layer within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Glacic', 'Glacic', 'Having a layer, 30 cm or more thick, containing 75 percent (by volume) or more icestarting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Garbic', 'Garbic', 'Having a layer, 20 cm or more thick within 100 cm of the soil surface, with20 percent or more (by volume, by weighted average) artefacts containing 35 percent or more(by volume) organic waste materials (in Technosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Fulvic', 'Fulvic', 'Having a fulvic horizon starting within 30 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Fragic', 'Fragic', 'Having a fragic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Grumic', 'Grumic', 'Having a soil surface layer with a thickness of 3 cm or more with a strongstructure finer than very coarse granular (in Vertisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Gypsiric', 'Gypsiric', 'Having gypsiric material between 20 and 50 cm from the soil surface or between20 cm and continuous rock or a cemented or indurated layer, whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hypereutric', 'Hypereutric', 'Having a base saturation (by 1 M NH 4 OAc) of 50 percent or morethroughout between 20 and 100 cm from the soil surface and 80 percent or more in somelayer within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Endogleyic', 'Endogleyic', 'Having between 50 and 100 cm from the mineral soil surface a layer, 25cm or more thick, that has reducing conditions in some parts and a gleyic colour pattern throughout', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hemic', 'Hemic', 'Having, after rubbing, between two-thirds and one-sixth (by volume) of theorganic material consisting of recognizable plant tissue within 100 cm from the soil surface (inHistosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Umbriglossic', 'Umbriglossic', 'Showing tonguing of an umbric horizon into an underlying layer', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Endoeutric', 'Endoeutric', 'Having a base saturation (by 1 M NH 4 OAc) of 50 percent or morethroughout between 50 and 100 cm from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Endofluvic', 'Endofluvic', 'Having fluvic material in a layer, 25 cm or more thick, between 50 and100 cm from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Epidystric', 'Epidystric', 'Having a base saturation (by 1 M NH 4 OAc) of less than 50 percentthroughout between 20 and 50 cm from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Epieutric', 'Epieutric', 'Having a base saturation (by 1 M NH 4 OAc) of 50 percent or morethroughout between 20 and 50 cm from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Rustic', 'Rustic', 'Having a spodic horizon in which the ratio of the percentage of acid oxalate (pH3)extractable Fe to the percentage of organic carbon is 6 or more throughout (in Podzols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Voronic', 'Voronic', 'Having a voronic horizon (in Chernozems only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Turbic', 'Turbic', 'Having cryoturbation features (mixed material, disrupted soil horizons, involutions,organic intrusions, frost heave, separation of coarse from fine materials, cracks or patternedground) at the soil surface or above a cryic horizon and within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Posic', 'Posic', 'Having a zero or positive charge (pH KCl - pH water ≥ 0, both in 1:1 solution) in a layer,30 cm or more thick, starting within 100 cm of the soil surface (in Plinthosols and Ferralsolsonly)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Leptic', 'Leptic', 'Having continuous rock starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Lixic', 'Lixic', 'Having an argic horizon that has a CEC (by 1 M NH 4 OAc) of less than 24 cmol c kg -1clay in some part to a maximum depth of 50 cm below its upper limit, either startingwithin 100 cm of the soil surface or within 200 cm of the soil surface if the argic horizon isoverlain by loamy sand or coarser textures throughout, and having a base saturation (by 1 MNH 4 OAc) of 50 percent or more in the major part between 50 and 100 cm from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Vetic', 'Vetic', 'Having an ECEC (sum of exchangeable bases plus exchangeable acidity in 1 M KCl)of less than 6 cmol c kg -1 clay in some subsurface layer within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Toxic', 'Toxic', 'Having in some layer within 50 cm of the soil surface toxic concentrations of organicor inorganic substances other than ions of Al, Fe, Na, Ca and Mg', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Magnesic', 'Magnesic', 'Having an exchangeable Ca to Mg ratio of less than 1 in the major part within100 cm of the soil surface or to continuous rock or a cemented or indurated layer, whichever isshallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Xanthic', 'Xanthic', 'Having a ferralic horizon that has in a subhorizon, 30 cm or more thick within150 cm of the soil surface, a Munsell hue of 7.5 YR or yellower and a value, moist, of 4 or moreand a chroma, moist, of 5 or more', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Stagnic', 'Stagnic', 'Having within 100 cm of the mineral soil surface in some parts reducing conditionsfor some time during the year and in 25 percent or more of the soil volume, single or incombination, a stagnic colour pattern or an albic horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Pisoplinthic', 'Pisoplinthic', 'Having a pisoplinthic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hyposalic', 'Hyposalic', 'Having an EC e of 4 dS m -1 or more at 25 ºC in some layer within 100 cmof the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Puffic', 'Puffic', 'Having a crust pushed up by salt crystals (in Solonchaks only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Mesotrophic', 'Mesotrophic', 'Having a base saturation (by 1 M NH 4 OAc) of less than 75 percent at adepth of 20 cm from the soil surface (in Vertisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Mazic', 'Mazic', 'Massive and hard to very hard in the upper 20 cm of the soil (in Vertisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Plaggic', 'Plaggic', 'Having a plaggic horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hydric', 'Hydric', 'Having within 100 cm of the soil surface one or more layers with a combinedthickness of 35 cm or more, which have a water retention at 1 500 kPa (in undried samples) of100 percent or more (in Andosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Pachic', 'Pachic', 'Having a mollic or umbric horizon 50 cm or more thick', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Aceric', 'Aceric', 'Having a pH (1:1 in water) between 3.5 and 5 and jarosite mottles in some layerwithin 100 cm of the soil surface (in Solonchaks only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Albic', 'Albic', 'Having an albic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hortic', 'Hortic', 'Having a hortic horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Ekranic', 'Ekranic', 'Having technic hard rock starting within 5 cm of the soil surface and covering95 percent or more of the horizontal extent of the soil (in Technosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Ombric', 'Ombric', 'Having a histic horizon saturated predominantly with rainwater starting within 40cm of the soil surface (in Histosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Escalic', 'Escalic', 'Occurring in human-made terraces', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Sodic', 'Sodic', 'Having 15 percent or more exchangeable Na plus Mg on the exchange complexwithin 50 cm of the soil surface throughout', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Haplic', 'Haplic', 'Having a typical expression of certain features (typical in the sense that there is nofurther or meaningful characterization) and only used if none of the preceding qualifiers applies', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Ornithic', 'Ornithic', 'Having a layer 15 cm or more thick with ornithogenic material starting within50 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hyperalbic', 'Hyperalbic', 'Having an albic horizon starting within 50 cm of the soil surface andhaving its lower boundary at a depth of 100 cm or more from the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Lignic', 'Lignic', 'Having inclusions of intact wood fragments, which make up one-quarter or more ofthe soil volume, within 50 cm of the soil surface (in Histosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Endosalic', 'Endosalic', 'Having a salic horizon starting between 50 and 100 cm from the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Linic', 'Linic', 'Having a continuous, very slowly permeable to impermeable constructedgeomembrane of any thickness starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Endostagnic', 'Endostagnic', 'Having between 50 and 100 cm from the mineral soil surface in someparts reducing conditions for some time during the year and in 25 percent or more of thesoil volume, single or in combination, a stagnic colour pattern or an albic horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Natric', 'Natric', 'Having a natric horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Vitric', 'Vitric', 'Having within 100 cm of the soil surface one or more layers with andic or vitricproperties with a combined thickness of 30 cm or more (in Cambisols: 15 cm or more), ofwhich 15 cm or more have vitric properties', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Histic', 'Histic', 'Having a histic horizon starting within 40 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hypergypsic', 'Hypergypsic', 'Having a gypsic horizon with 50 percent or more (by mass) gypsum andstarting within 100 cm of the soil surface (in Gypsisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Endoleptic', 'Endoleptic', 'Having continuous rock starting between 50 and 100 cm from the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Petroplinthic', 'Petroplinthic', 'Having a petroplinthic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Spolic', 'Spolic', 'Having a layer, 20 cm or more thick within 100 cm of the soil surface, with20 percent or more (by volume, by weighted average) artefacts containing 35 percent or more(by volume) of industrial waste (mine spoil, dredgings, rubble, etc. (in Technosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Luvic', 'Luvic', 'Having an argic horizon that has a CEC (by 1 M NH 4 OAc) of 24 cmol c kg -1 clay ormore throughout or to a depth of 50 cm below its upper limit, whichever is shallower, eitherstarting within 100 cm of the soil surface or within 200 cm of the soil surface if the argichorizon is overlain by loamy sand or coarser textures throughout, and having a base saturation(by 1 M NH 4 OAc) of 50 percent or more in the major part between 50 and 100 cm from the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Lamellic', 'Lamellic', 'Having clay lamellae with a combined thickness of 15 cm or more within 100 cmof the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Melanic', 'Melanic', 'Having a melanic horizon starting within 30 cm of the soil surface (in Andosolsonly)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Ruptic', 'Ruptic', 'Having a lithological discontinuity within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hydragric', 'Hydragric', 'Having an anthraquic horizon and an underlying hydragric horizon, the latterstarting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Petric', 'Petric', 'Having a strongly cemented or indurated layer starting within 100 cm of the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Protic', 'Protic', 'Showing no soil horizon development (in Arenosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Thionic', 'Thionic', 'Having a thionic horizon or a layer with sulphidic material, 15 cm or more thick,starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Spodic', 'Spodic', 'Having a spodic horizon starting within 200 cm of the mineral soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Petroduric', 'Petroduric', 'Having a petroduric horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Subaquatic', 'Subaquatic', 'Being permanently submerged under water not deeper than 200 cm (inFluvisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Technic', 'Technic', 'Having 10 percent or more (by volume, by weighted average) artefacts in theupper 100 cm from the soil surface or to continuous rock or a cemented or indurated layer,whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Reductic', 'Reductic', 'Having reducing conditions in 25 percent or more of the soil volume within 100cm of the soil surface caused by gaseous emissions, e.g. methane or carbon dioxide (inTechnosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Takyric', 'Takyric', 'Having a takyric horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Transportic', 'Transportic', 'Having at the surface a layer, 30 cm or more thick, with solid or liquidmaterial that has been moved from a source area outside the immediate vicinity of the soil byintentional human activity, usually with the aid of machinery, and without substantial reworkingor displacement by natural forces', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Vermic', 'Vermic', 'Having 50 percent or more (by volume, by weighted average) of worm holes,casts, or filled animal burrows in the upper 100 cm of the soil or to continuous rock or acemented or indurated layer, whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Plinthic', 'Plinthic', 'Having a plinthic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hypocalcic', 'Hypocalcic', 'Having a calcic horizon with a calcium carbonate equivalent content in thefine earth fraction of less than 25 percent and starting within 100 cm of the soil surface (inCalcisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Rubic', 'Rubic', 'Having within 100 cm of the soil surface a subsurface layer, 30 cm or more thick,with a Munsell hue redder than 10 YR or a chroma, moist, of 5 or more (in Arenosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Nitic', 'Nitic', 'Having a nitic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Thaptovitric', 'Thaptovitric', 'Having within 100 cm of the soil surface one or more buried layerswith andic or vitric properties with a combined thickness of 30 cm or more (in Cambisols: 15 cm or more), of which 15 cm or more havevitric properties', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Tephric', 'Tephric', 'Having tephric material to a depth of 30 cm or more from the soil surface or tocontinuous rock, whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hyperskeletic', 'Hyperskeletic', 'Containing less than 20 percent (by volume) fine earth averaged over adepth of 75 cm from the soil surface or to continuous rock, whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Lithic', 'Lithic', 'Having continuous rock starting within 10 cm of the soil surface (in Leptosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Tidalic', 'Tidalic', 'Being flooded by tidewater but not covered by water at mean low tide', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Sulphatic', 'Sulphatic', 'Having a salic horizon with a soil solution (1:1 in water) with [SO 42- ] >> [HCO 3-] > [Cl - ] (in Solonchaks only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Profondic', 'Profondic', 'Having an argic horizon in which the clay content does not decrease by20 percent or more (relative) from its maximum within 150 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Terric', 'Terric', 'Having a terric horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Placic', 'Placic', 'Having, within 100 cm of the soil surface, an iron pan, between 1 and 25 mm thick,that is continuously cemented by a combination of organic matter, Fe and/or Al', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Thixotropic', 'Thixotropic', 'Having in some layer within 50 cm of the soil surface material that changes,under pressure or by rubbing, from a plastic solid into a liquefied stage and back into the solidcondition', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Regic', 'Regic', 'Not having buried horizons (in Anthrosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Petrocalcic', 'Petrocalcic', 'Having a petrocalcic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Protothionic', 'Protothionic', 'Having a layer with sulphidic material, 15 cm or more thick, startingwithin 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Skeletic', 'Skeletic', 'Having 40 percent or more (by volume) gravel or other coarse fragments averagedover a depth of 100 cm from the soil surface or to continuous rock or a cemented or induratedlayer, whichever is shallower', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Manganiferric', 'Manganiferric', 'Having a ferric horizon starting within 100 cm of the soil surface in whichhalf or more of the nodules or mottles are black', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hydrophobic', 'Hydrophobic', 'Water-repellent, i.e. water stands on a dry soil for the duration of 60 secondsor more (in Arenosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Oxyaquic', 'Oxyaquic', 'Saturated with oxygen-rich water during a period of 20 or more consecutivedays and not having a gleyic or stagnic colour pattern in some layer within 100 cm of the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Rendzic', 'Rendzic', 'Having a mollic horizon that contains or immediately overlies calcaric materialsor calcareous rock containing 40 percent or more calcium carbonate equivalent', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Humic', 'Humic', 'Having the following organic carbon contents in the fine earth fraction as aweighted average: in Ferralsols and Nitisols, 1.4 percent or more to a depth of 100 cm from themineral soil surface; in Leptosols to which the Hyperskeletic qualifier applies, 2 percent or moreto a depth of 25 cm from the mineral soil surface; in other soils, 1 percent or more to a depth of50 cm from the mineral soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Laxic', 'Laxic', 'Having a bulk density of less than 0.89 kg dm -3 , in a mineral soil layer, 20 cm ormore thick, starting within 75 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Limnic', 'Limnic', 'Having limnic material, cumulatively 10 cm or more thick, within 50 cm of thesoil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hyperalic', 'Hyperalic', 'Having an argic horizon, either starting within 100 cm of the soil surface orwithin 200 cm of the soil surface if the argic horizon is overlain by loamy sand or coarsertextures throughout, that has a silt to clay ratio of less than 0.6 and an Al saturation (effective)of 50 percent or more, throughout or to a depth of 50 cm below its upper limit, whichever isshallower (in Alisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Yermic', 'Yermic', 'Having a yermic horizon, including a desert pavement', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Petrosalic', 'Petrosalic', 'Having, within 100 cm of the soil surface, a layer, 10 cm or more thick, whichis cemented by salts more soluble than gypsum', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Nudilithic', 'Nudilithic', 'Having continuous rock at the soil surface (in Leptosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Petrogleyic', 'Petrogleyic', 'Having a layer, 10 cm or more thick, with an oximorphic colour pattern 59 ,15 percent or more (by volume) of which is cemented (bog iron), within 100 cm of the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Rhodic', 'Rhodic', 'Having within 150 cm of the soil surface a subsurface layer, 30 cm or more thick,with a Munsell hue of 2.5 YR or redder, a value, moist, of less than 3.5 anda value, dry, no more than one unit higher than the moist value', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Siltic', 'Siltic', 'Having a texture of silt, silt loam, silty clay loam or silty clay in a layer, 30 cm ormore thick, within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Silandic', 'Silandic', 'Having one or more layers, cumulatively 15 cm or more thick, with andicproperties and an acid oxalate (pH 3) extractable silica (Si ox ) content of 0.6 percent or more, oran Al py to Al ox ratio of less than 0.5 within 100 cm of the soil surface (in Andosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hyperochric', 'Hyperochric', 'Having a mineral topsoil layer, 5 cm or more thick, with a Munsell value,dry, of 5.5 or more that turns darker on moistening, an organic carbon content of less than0.4 percent, a platy structure in 50 percent or more of the volume, and a surface crust', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Urbic', 'Urbic', 'Having a layer, 20 cm or more thick within 100 cm of the soil surface, with20 percent or more (by volume, by weighted average) artefacts containing 35 percent or more(by volume) of rubble and refuse of human settlements (in Technosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Reductaquic', 'Reductaquic', 'Saturated with water during the thawing period and having at some time ofthe year reducing conditions above a cryic horizon and within 100 cm of the soil surface (inCryosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hyperdystric', 'Hyperdystric', 'Having a base saturation (by 1 M NH 4 OAc) of less than 50 percentthroughout between 20 and 100 cm from the soil surface, and less than 20 percent insome layer within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Petrogypsic', 'Petrogypsic', 'Having a petrogypsic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hypogypsic', 'Hypogypsic', 'Having a gypsic horizon with a gypsum content in the fine earth fraction ofless than 25 percent and starting within 100 cm of the soil surface (in Gypsisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Vertic', 'Vertic', 'Having a vertic horizon or vertic properties starting within 100 cm of the soilsurface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Pellic', 'Pellic', 'Having in the upper 30 cm of the soil a Munsell value, moist, of 3.5 or less and achroma, moist, of 1.5 or less (in Vertisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Sapric', 'Sapric', 'Having, after rubbing, less than one-sixth (by volume) of the organic materialconsisting of recognizable plant tissue within 100 cm of the soil surface (in Histosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Novic', 'Novic', 'Having above the soil that is classified at the RSG level, a layer with recentsediments (new material), 5 cm or more and less than 50 cm thick', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hypersalic', 'Hypersalic', 'Having an EC e of 30 dS m -1 or more at 25 ºC in some layer within100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hypoluvic', 'Hypoluvic', 'Having an absolute clay increase of 3 percent or more within 100 cm of the soilsurface (in Arenosols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Mollic', 'Mollic', 'Having a mollic horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Salic', 'Salic', 'Having a salic horizon starting within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Umbric', 'Umbric', 'Having an umbric horizon', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hypercalcic', 'Hypercalcic', 'Having a calcic horizon with 50 percent or more (by mass) calcium carbonateequivalent and starting within 100 cm of the soil surface (in Calcisols only)', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Hyposodic', 'Hyposodic', 'Having 6 percent or more exchangeable Na on the exchange complexin a layer, 20 cm or more thick, within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Abruptic', 'Abruptic', 'Having an abrupt textural change within 100 cm of the soil surface', 'WRBQualifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBQualifierValue/Entic', 'Entic', 'Not having an albic horizon and having a loose spodic horizon (in Podzols only)', 'WRBQualifierValue', null, null, null, null);
...
-- FEATURE wrbqualifiergrouptype
-- so.codelist INSPIRE 
-- http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue (Under review)

INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/bathi', 'Bathi', 'Bathi', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/cumuli', 'Cumuli', 'Cumuli', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/endo', 'Endo', 'Endo', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/epi', 'Epi', 'Epi', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/hyper', 'Hyper', 'Hyper', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/hypo', 'Hypo', 'Hypo', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/ortho', 'Ortho', 'Ortho', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/para', 'Para', 'Para', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/proto', 'Proto', 'Proto', 'WRBSpecifierValue', null, null, null, null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/WRBSpecifierValue/thapto', 'Thapto', 'Thapto', 'WRBSpecifierValue', null, null, null, null);
...
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/metadata-so.codelist/ResponsiblePartyRole
...
-- PARAMETER soilsite
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue

-- CHEMICAL
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalAs', 'Arsenic and compounds (as As)', 'as in E-PRTR, CAS-Nr.: 7440-38-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalBa', 'Barium and compounds (as Ba)', 'CAS-Nr.: 82870-81-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalCd', 'Cadmium and compounds (as Cd)', 'as in E-PRTR, CAS-Nr.: 7440-43-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalCr', 'Chromium and compounds (as Cr)', 'as in E-PRTR, CAS-Nr.: 7440-47-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalCo', 'Cobalt and compounds (as Co)', 'CAS-Nr.: 7440-48-4', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalCu', 'Copper and compounds (as Cu)', 'as in E-PRTR, CAS-Nr.: 7440-50-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalHg', 'Mercury and compounds (as Hg)', 'as in E-PRTR, CAS-Nr.: 7439-97-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalNi', 'Nickel and compounds (as Ni)', 'as in E-PRTR, CAS-Nr.: 7440-02-0', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalPb', 'Lead and compounds (as Pb)', 'as in E-PRTR, CAS-Nr.: 7439-92-1', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalTl', 'Thallium and compounds (as Tl)', 'CAS-Nr.: 82870-81-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalZn', 'Zinc and compounds (as Zn)', 'as in E-PRTR, CAS-Nr.: 7440-66-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalSb', 'Antimony and compounds (as Sb)', 'CAS-Nr.: 7440-36-0', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalV', 'Vanadium and compounds (as V)', 'CAS-Nr.: 7440-62-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/metalMo', 'Molybdenum and compounds (as Mo)', 'CAS-Nr.: 7439-89-7', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/organometalSn', 'Organotin compounds (as total Sn)', 'as in E-PRTR, CAS-Nr.: 7440-31-5', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/organometalTributylSn', 'Tributyltin and compounds (total mass)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/organometalTriphenylSn', 'Triphenyltin and compounds (total mass)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/inorganicAsbestos', 'Asbestos', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/inorganicCN', 'Cyanides (as total CN)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/inorganicF', 'Fluorides (as total F)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/aromaticBTEX', 'BTEX', 'as in E-PRTR,  Sum of benzene, toluene. Ethylbenzene and Xylenes', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/aromaticBenzene', 'Benzene', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/aromaticToluene', 'Toluene', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/aromaticEthylbenzene', 'Ethylbenzene', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/aromaticXylene', 'Xylene', 'as in E-PRTR, sum of 3 isomers', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/aromaticStyrene', 'Styrene', 'Styrene', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCBs', 'Polychlorinated biphenyls (PCBs)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB28', 'Polychlorinated biphenyl 28', 'CAS-Nr.: 7012-37-5', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB52', 'Polychlorinated biphenyls 52', 'CAS-Nr.: 35693-99-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB101', 'Polychlorinated biphenyls 101', 'CAS-Nr.: 37680-73-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB138', 'Polychlorinated biphenyls 138', 'CAS-Nr.: 35065-28-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB153', 'Polychlorinated biphenyls 153', 'CAS-Nr.: 35065-27-1', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB180', 'Polychlorinated biphenyls 180', 'CAS-Nr.: 35065-29-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB77', 'Polychlorinated biphenyls 77', 'as in POP convention, CAS-Nr.: 1336-36-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB81', 'Polychlorinated biphenyls 81', 'as in POP convention, CAS-Nr.: 70362-50-4', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB126', 'Polychlorinated biphenyls 126', 'as in POP convention, CAS-Nr.: 57465-288', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB169', 'Polychlorinated biphenyls 169', 'as in POP convention, CAS-Nr.: 32774-16-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB105', 'Polychlorinated biphenyls 105', 'as in POP convention, CAS-Nr.: 32598-14-4', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB114', 'Polychlorinated biphenyls 114', 'as in POP convention, CAS-Nr.: 74472-37-0', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB118', 'Polychlorinated biphenyls 118', 'as in POP convention, CAS-Nr.: 31508-00-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB123', 'Polychlorinated biphenyls 123', 'as in POP convention, CAS-Nr.: 65510-44-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB156', 'Polychlorinated biphenyls 156', 'as in POP convention, CAS-Nr.: 38380-08-4', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB157', 'Polychlorinated biphenyls 157', 'as in POP convention, CAS-Nr.: 69782-90-7', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB167', 'Polychlorinated biphenyls 167', 'as in POP convention, CAS-Nr.: 52663-72-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCB189', 'Polychlorinated biphenyls 189', 'as in POP convention, CAS-Nr.: 39635-31-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticHCB', 'Hexachlorobenzene (HCB)', 'as in E-PRTR, CAS-Nr.: 118-74-1', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPCDD-PCF', 'PCDD+PCDF (dioxines and furans; as Teq)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic2-3-7-8-Tetra-CDD', '2,3,7,8-Tetra-CDD (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-7-8-Penta-CDD', '1,2,3,7,8-Penta-CDD (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-4-7-8-Hexa-CDD', '1,2,3,4,7,8-Hexa-CDD (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-6-7-8-Hexa-CDD', '1,2,3,6,7,8-Hexa-CDD (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-7-8-9-Hexa-CDD', '1,2,3,7,8,9-Hexa-CDD (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-3-6-7-8-Hepta-CDD', '1,2,3,3,6,7,8-Hepta-CDD (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-4-6-7-8-9-Octa-CDD', '1,2,3,4,6,7,8,9-Octa-CDD (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic2-3-7-8-Tetra-CDF', '2,3,7,8-Tetra-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-7-8-Penta-CDF', '1,2,3,7,8-Penta-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic2-3-4-7-8-Penta-CDF', '2,3,4,7,8-Penta-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-4-7-8-Hexa-CDF', '1,2,3,4,7,8-Hexa-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-6-7-8-Hexa-CDF', '1,2,3,6,7,8-Hexa-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-7-8-9-Hexa-CDF', '1,2,3,7,8,9-Hexa-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic2-3-4-6-7-8-Hexa-CDF', '2,3,4,6,7,8-Hexa-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-4-6-7-8-Hepta-CDF', '1,2,3,4,6,7,8-Hepta-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-4-7-8-9-Hepta-CDF', '1,2,3,4,7,8,9-Hepta-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic1-2-3-4-6-7-8-9-Octa-CDF', '1,2,3,4,6,7,8,9-Octa-CDF (as concentration ng/kg)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticClbenzenes', 'Chlorobenzenes (total)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticTriClbenzenes', 'Trichlorobenzenes', 'Chlorobenzenes (total)', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticPentaClbenzene', 'Pentachlorobenzene', 'as in E-PRTR, CAS-Nr.: 608-93-5', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticHCBD', 'Hexachlorobutadiene (HCBD)', 'as in E-PRTR, CAS-Nr.: 87-68-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticHBB', 'Hexabromobiphenyl (HBB)', 'as in E-PRTR, CAS-Nr.: 36355-1-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromaticBDPE', 'Brominated diphenylether (sum) / Pentabromodiphenylether', 'as in priority substances EU water policy, CAS-Nr.: ../32534-81-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic6-7BDPE', 'Hexabromodiphenyl ether and heptabromodiphenyl ether', 'as in POP convention', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAromatic4-5BDPE', 'Tetrabromodiphenyl ether and Pentabromodiphenyl ether', 'as in POP convention', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticAOX', 'halogenated organic compounds (as AOX)', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticC10-13', 'Chloro-alkanes C10-C13', 'as in priority substances EU water policy, CAS-Nr.: 85535-84-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticTCE', '{Trichloroethylene}', 'as in E-PRTR,CAS-Nr.:  79-01-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticPCE', '{Tetrachloroethylene (or Perchloroethylene)}', 'as in E-PRTR, CAS-Nr.: 127-18-4', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticDCM', 'Dichloromethane (DCM)', 'as in E-PRTR, CAS-Nr.: 75-09-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticTriCM', '{Trichloromethane (chloroform)}', 'as in E-PRTR, CAS-Nr.: 67-66-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticEDC', '1,2-dichlorethane (EDC)', 'as in E-PRTR, CAS-Nr.: 107-06-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticTCM', '{Tetrachloromethane (TCM)}', 'as in E-PRTR, CAS-Nr.: 56-23-5', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticVinylCl', 'Vinylchloride', 'as in E-PRTR, CAS-Nr.: 75-01-4', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/haloAliphaticPFOS-A', 'Perfluorooctane sulfonic (acid and salts) and Perfluorooctane sulfonyl fluoride', 'as in E-PRTR,', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/phenolsTotal', 'Phenols (as total C of phenols)', 'as in E-PRTR,  108-95-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/phenolsPCP', 'Pentachlorophenol (PCP)', 'as in E-PRTR, 87-86-5', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/phenolsClPTotal', 'Chlorophenols (total)', 'Chlorophenols (total)', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/phenolsNonylP', 'Nonylphenols / (4-nonylphenol)', 'as in priority substances EU water policy, CAS-Nr.: 25154-52-3/(104-40-5)', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/phenolsOctylP', '{Octylphenols and octylphenolethoxylates}', 'as in E-PRTR, CAS-Nr.: 1806-26-4/ 140-66-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAHsum', 'PAHs sum or report specific releases of', 'as in E-PRTR', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-BaP', 'Benzo(a)pyrene', 'as in E-PRTR, CAS-Nr.: 50-32-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-BbF', 'Benzo(b)fluoranthene', 'as in E-PRTR, CAS-Nr.: 205-99-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-BkF', 'Benzo(k)fluoranthene', 'as in E-PRTR, CAS-Nr.: 207-08-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-IcP', 'Indeno(1,23-cd)pyrene', 'as in E-PRTR, CAS-Nr.: 193-39-5', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-BgP', '{Benzo(g,h,i)perylene}', 'as in E-PRTR, CAS-Nr.: 191-24-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-ANT', 'Anthracene', 'as in E-PRTR, CAS-Nr.: 120-12-7', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-NAP', 'Naphtalene', 'as in E-PRTR, CAS-Nr.: 91-20-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-ACY', 'Acenaphthylene', 'CAS-Nr.: 208-96-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-ACE', 'Acenaphthene', 'CAS-Nr.: 83-32-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-FLE', 'Fluorene', 'CAS-Nr.: 86-73-7', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-PHE', 'Phenanthrene', 'CAS-Nr.: 85-01-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-FLA', 'Fluoranthene', 'CAS-Nr.: 206-44-0', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-PYE', 'Pyrene', 'CAS-Nr.: 129-00-0', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-BaA', 'Benzo(a)anthracene', 'CAS-Nr.: 56-55-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-CHE', 'Chrysene', 'CAS-Nr.: 218-01-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/PAH-DaA', 'Dibenzo(a,h)anthracene', 'CAS-Nr.: 53-70-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideAldrin', 'Aldrin', 'as in E-PRTR, CAS-Nr.: 309-00-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideDieldrin', 'Dieldrin', 'as in E-PRTR, CAS-Nr.: 60-57-1', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideEndrin', 'Endrin', 'as in E-PRTR, CAS-Nr.: 72-20-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideIsodrin', '{Isodrin}', 'as in E-PRTR, 465-73-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideOpDDT', 'op-DDT', 'CAS-Nr.: 789-02-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticidePpDDT', 'pp-DDT', 'CAS-Nr.: 50-29-3', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideAlHCH', 'alpha-HCH', 'CAS-Nr.: 319-84-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideBeHCH', 'beta-HCH', 'CAS-Nr.: 319-85-7', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideDeHCH', 'delta-HCH', 'CAS-Nr.: 319-86-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideGaHCH', 'gamma-HCH (Lindan)', 'as in E-PRTR, CAS-Nr.: 58-89-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideAtrazin', 'Atrazine', 'as in E-PRTR, 1912-24-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideChlordane', 'Chlordane', 'as in E-PRTR, 57-74-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideChlordecone', 'Chlordecone', 'as in E-PRTR, CAS-Nr.:143-50-0', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideChlorfenvinphos', 'Chlorfenvinphos', 'as in E-PRTR, CAS-Nr.:470-90-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideChlorpyrifos', 'Chlorpyrifos', 'as in E-PRTR, CAS-Nr.:2921-88-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideDiuron', 'Diuron', 'as in E-PRTR, CAS-Nr.:330-54-1', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideEndosulphan', 'Endosulphan', 'as in E-PRTR, CAS-Nr.:115-29-7', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideHeptachlor', 'Heptachlor', 'as in E-PRTR, CAS-Nr.:76-44-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideMirex', 'Mirex', 'as in E-PRTR, CAS-Nr.:2385-85-5', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideSimazine', 'Simazine', 'as in E-PRTR, CAS-Nr.:122-34-9', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideToxaphene', 'Toxaphene', 'as in E-PRTR, CAS-Nr.:8001-35-2', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideIsoproturon', 'Isoproturon', 'as in E-PRTR, CAS-Nr.:34123-59-6', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideDEHP', 'Di-(2-ethyl hexyl) phtalate (DEHP)', 'as in priority substances EU water policy, CAS-Nr.:117-81-7', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideTrifluralin', 'Trifluralin', 'as in E-PRTR, CAS-Nr.:1582-09-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideAlachlor', 'Alachlor', 'as in E-PRTR, CAS-Nr.:15972-60-8', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/pesticideCyclodiene', 'Cyclodiene pesticides', 'as in priority substances EU water policy', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/otherMTBE', 'Methyl tertiary-butyl ether (MTBE)', 'CAS-Nr.:1634-04-4', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/otherMineralOil', 'Mineral oil', 'Mineral oil', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilSiteParameterNameValue/otherPhtalatesTotal', 'Phtalates (total)', 'Phtalates (total)', 'SoilSiteParameterNameValue', 'soilsite', 'chemical', 'soilsitechemical', null);


-- SoilProfileParameterNameValue
-- PARAMETER soilprofile
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/SoilProfileParameterNameValue

-- CHEMICAL
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilProfileParameterNameValue/carbonStock', 'carbon stock', 'The total mass of carbon in soil for a given depth.', 'SoilProfileParameterNameValue', 'soilprofile', 'chemical', 'soilprofilechemical', null);
-- PHYSICAL
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilProfileParameterNameValue/potentialRootDepth', 'potential root depth', 'Potential depth of the soil profile where roots develop (in cm).', 'SoilProfileParameterNameValue', 'soilprofile', 'physical', 'soilprofilephysical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilProfileParameterNameValue/availableWaterCapacity', 'available water capacity', 'Amount of water that a soil can store that is usable by plants, based on the potential root depth.', 'SoilProfileParameterNameValue', 'soilprofile', 'physical', 'soilprofilephysical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilProfileParameterNameValue/waterDrainage', 'water drainage', 'Natural internal water drainage class of the soil profile.', 'SoilProfileParameterNameValue', 'soilprofile', 'physical', 'soilprofilephysical', null);


-- SoilDerivedObjectParameterNameValue
-- PARAMETER soilderivedobject
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue

-- CHEMICAL
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/carbonStock', 'carbon stock', 'The total mass of carbon in soil for a given depth.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/organicCarbonContent', 'organic carbon content', 'Portion of the soil measured as carbon in organic form, excluding living macro and mesofauna and living plant tissue.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/nitrogenContent', 'nitrogen content', 'Total nitrogen content in the soil, including both the organic and inorganic forms.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/pHValue', 'pH value', 'pH value of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/cadmiumContent', 'cadmium content', 'Cadmium content of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/chromiumContent', 'chromium content', 'Chromium content of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/copperContent', 'copper content', 'Copper content of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/leadContent', 'lead content', 'Lead content of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/mercuryContent', 'mercury content', 'Mercury content of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/nickelContent', 'nickel content', 'Nickel content of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/zincContent', 'zinc content', 'Zinc content of the soil derived object.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'chemical', 'soilderivedobjectchemical', null);
-- PHYSICAL
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/potentialRootDepth', 'potential root depth', 'Potential depth of the soil profile where roots develop (in cm).', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'physical', 'soilderivedobjectphysical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/availableWaterCapacity', 'available water capacity', 'Amount of water that a soil can store that is usable by plants, based on the potential root depth.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'physical', 'soilderivedobjectphysical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/SoilDerivedObjectParameterNameValue/waterDrainage', 'water drainage', 'Natural water drainage class of the soil profile.', 'SoilDerivedObjectParameterNameValue', 'soilderivedobject', 'physical', 'soilderivedobjectphysical', null);


-- ProfileElementParameterNameValue
-- PARAMETER profileelement
-- so.codelist INSPIRE
-- http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue

-- CHEMICAL
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/organicCarbonContent', 'organic carbon content', 'Portion of the soil measured as carbon in organic forms, excluding living macro and mesofauna and living plant tissue.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/nitrogenContent', 'nitrogen content', 'total nitrogen content in the soil, including both the organic and inorganic forms.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/pHValue', 'pH value', 'pH value of the profile element.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/cadmiumContent', 'cadmium content', 'Cadmium content of the profile element.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/chromiumContent', 'chromium content', 'Chromium content of the profile element.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/copperContent', 'copper content', 'Copper content of the profile element.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/leadContent', 'lead content', 'Lead content of the profile element.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/mercuryContent', 'mercury content', 'Mercury content of the profile element.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
INSERT INTO so.codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/so.codelist/ProfileElementParameterNameValue/nickelContent', 'nickel content', 'Nickel content of the profile element.', 'ProfileElementParameterNameValue', 'profileelement', 'chemical', 'profileelementchemical', null);
