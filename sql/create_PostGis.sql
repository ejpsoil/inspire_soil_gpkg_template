...
COMMENT ON TABLE soilsite IS 'Area within a larger survey, study or monitored area, where a specific soil investigation is carried out.';
-- Add Comment Column
--COMMENT ON COLUMN soilsite.id IS 'Primary Key of the Table';
COMMENT ON COLUMN soilsite.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN soilsite.inspireid_localid IS 'A local identifier, assigned by the data provider. The local identifier is unique within the namespace, that is no other spatial object carries the same unique identifier.';
COMMENT ON COLUMN soilsite.inspireid_namespace IS 'Namespace uniquely identifying the data source of the spatial object.';
COMMENT ON COLUMN soilsite.inspireid_versionid IS 'The identifier of the particular version of the spatial object, with a maximum length of 25 characters. If the specification of a spatial object type with an external object identifier includes life-cycle information, the version identifier is used to distinguish between the different versions of a spatial object. Within the set of all versions of a spatial object, the version identifier is unique.';
COMMENT ON COLUMN soilsite.soilinvestigationpurpose IS 'Indication why a survey was conducted.';
COMMENT ON COLUMN soilsite.validfrom IS 'The time when the phenomenon started to exist in the real world.';
COMMENT ON COLUMN soilsite.validto IS 'The time from which the phenomenon no longer exists in the real world.';
COMMENT ON COLUMN soilsite.beginlifespanversion IS 'Date and time at which this version of the spatial object was inserted or changed in the spatial data set.';
COMMENT ON COLUMN soilsite.endlifespanversion IS 'Date and time at which this version of the spatial object was superseded or retired in the spatial data set.';
COMMENT ON COLUMN soilsite.geometry IS 'Geometry';

-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_valid_period_soilsite()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON soilsite
FOR EACH ROW
EXECUTE FUNCTION check_valid_period_soilsite();


CREATE OR REPLACE FUNCTION check_valid_versions_soilsite()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON soilsite
FOR EACH ROW
EXECUTE FUNCTION check_valid_versions_soilsite();


CREATE OR REPLACE FUNCTION check_soil_investigation_purpose()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON soilsite
FOR EACH ROW
EXECUTE FUNCTION check_soil_investigation_purpose();


CREATE OR REPLACE FUNCTION update_begin_today_soilsite()
RETURNS TRIGGER AS $$
BEGIN
...
ON soilsite
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilsite();

CREATE OR REPLACE FUNCTION update_begin_today_soilsite_error()
RETURNS TRIGGER AS $$
BEGIN
...
ON soilsite
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilsite_error();
...
(
    ...
    beginlifespanversion TIMESTAMPTZ DEFAULT current_timestamp not null,
    endlifespanversion TIMESTAMPTZ,
    locatedon UUID REFERENCES soilsite(guidkey) ON UPDATE CASCADE
);

-- Add Geometry Column
--SELECT AddGeometryColumn('public', 'soilplot', 'soilplotlocation', 3035, 'POINT', 2);

COMMENT ON TABLE soilplot IS 'Spot where a specific soil investigation is carried out.';
-- Add Comment Column
--COMMENT ON COLUMN soilplot.id IS 'Primary Key of the Table';
COMMENT ON COLUMN soilplot.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN soilplot.inspireid_localid IS 'A local identifier, assigned by the data provider. The local identifier is unique within the namespace, that is no other spatial object carries the same unique identifier.';
COMMENT ON COLUMN soilplot.inspireid_namespace IS 'Namespace uniquely identifying the data source of the spatial object.';
COMMENT ON COLUMN soilplot.inspireid_versionid IS 'The identifier of the particular version of the spatial object, with a maximum length of 25 characters. If the specification of a spatial object type with an external object identifier includes life-cycle information, the version identifier is used to distinguish between the different versions of a spatial object. Within the set of all versions of a spatial object, the version identifier is unique.';
COMMENT ON COLUMN soilplot.soilplottype IS 'Gives information on what kind of plot the observation of the soil is made on.';
COMMENT ON COLUMN soilplot.beginlifespanversion IS 'Date and time at which this version of the spatial object was inserted or changed in the spatial data set.';
COMMENT ON COLUMN soilplot.endlifespanversion IS 'Date and time at which this version of the spatial object was superseded or retired in the spatial data set.';
COMMENT ON COLUMN soilplot.locatedon IS 'Foreign key to the SoilSite table, guidkey field.';
COMMENT ON COLUMN soilplot.soilplotlocation IS 'Geometry';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_valid_version_soilplot()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.beginlifespanversion > NEW.endlifespanversion THEN
...
BEFORE INSERT OR UPDATE ON soilplot
FOR EACH ROW
EXECUTE FUNCTION check_valid_version_soilplot();

CREATE OR REPLACE FUNCTION check_soilplot_type()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON soilplot
FOR EACH ROW
EXECUTE FUNCTION check_soilplot_type();

CREATE OR REPLACE FUNCTION update_begin_today_soilplot()
RETURNS TRIGGER AS $$
BEGIN
    IF CURRENT_TIMESTAMP < NEW.endlifespanversion OR NEW.endlifespanversion IS NULL THEN
        NEW.beginlifespanversion := CURRENT_TIMESTAMP;
...
ON soilplot
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilplot();

CREATE OR REPLACE FUNCTION update_begin_today_soilplot_error()
RETURNS TRIGGER AS $$
BEGIN
    IF CURRENT_TIMESTAMP > NEW.endlifespanversion THEN
        RAISE EXCEPTION 'If you change record endlifespanversion must be greater than today';
...
ON soilplot
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilplot_error();
...
COMMENT ON TABLE soilprofile IS 'Description of the soil that is characterized by a vertical succession of profile elements.';
-- Add Comment Column
-- COMMENT ON COLUMN soilprofile.id IS 'Primary Key of the Table';
COMMENT ON COLUMN soilprofile.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN soilprofile.inspireid_localid IS 'A local identifier, assigned by the data provider. The local identifier is unique within the namespace, that is no other spatial object carries the same unique identifier.';
COMMENT ON COLUMN soilprofile.inspireid_namespace IS 'Namespace uniquely identifying the data source of the spatial object.';
COMMENT ON COLUMN soilprofile.inspireid_versionid IS 'The identifier of the particular version of the spatial object, with a maximum length of 25 characters. If the specification of a spatial object type with an external object identifier includes life-cycle information, the version identifier is used to distinguish between the different versions of a spatial object. Within the set of all versions of a spatial object, the version identifier is unique.';
COMMENT ON COLUMN soilprofile.localidentifier IS 'Unique identifier of the soil profile given by the data provider of the dataset.';
COMMENT ON COLUMN soilprofile.beginlifespanversion IS 'Date and time at which this version of the spatial object was inserted or changed in the spatial data set.';
COMMENT ON COLUMN soilprofile.endlifespanversion IS 'Date and time at which this version of the spatial object was superseded or retired in the spatial data set.';
COMMENT ON COLUMN soilprofile.validfrom IS 'The time when the phenomenon started to exist in the real world.';
COMMENT ON COLUMN soilprofile.validto IS 'The time from which the phenomenon no longer exists in the real world.';
COMMENT ON COLUMN soilprofile.isderived IS 'Boolean value to indicate whether the record is of Derived or Observed type';
COMMENT ON COLUMN soilprofile.wrbversion IS 'Indicates the WRB classification version.';
COMMENT ON COLUMN soilprofile.wrbreferencesoilgroup IS 'First level of classification of the World Reference Base for Soil Resources.';
COMMENT ON COLUMN soilprofile.isoriginalclassification IS 'Boolean value to indicate whether the WRB classification system was the original classification system to describe the soil profile.';
COMMENT ON COLUMN soilprofile.location IS 'Foreign key to the SoilPlot table, guidkey field.';

-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_valid_period_soilprofile()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.validfrom > NEW.validto THEN
...
FOR EACH ROW
EXECUTE FUNCTION check_valid_period_soilprofile();


CREATE OR REPLACE FUNCTION check_valid_version_soilprofile()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.beginlifespanversion >= NEW.endlifespanversion THEN
...
BEFORE INSERT OR UPDATE ON soilprofile
FOR EACH ROW
EXECUTE FUNCTION check_valid_version_soilprofile();


CREATE OR REPLACE FUNCTION check_profile_location() 
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON soilprofile
FOR EACH ROW
EXECUTE FUNCTION check_profile_location();


CREATE OR REPLACE FUNCTION check_profile_location_observed()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON soilprofile
FOR EACH ROW
EXECUTE FUNCTION check_profile_location_observed();
...
CREATE OR REPLACE FUNCTION update_begin_today_soilprofile()
RETURNS TRIGGER AS $$
BEGIN
    IF CURRENT_TIMESTAMP < NEW.endlifespanversion OR NEW.endlifespanversion IS NULL THEN
        NEW.beginlifespanversion := CURRENT_TIMESTAMP;
...
ON soilprofile
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilprofile();


CREATE OR REPLACE FUNCTION update_begin_today_soilprofile_error()
RETURNS TRIGGER AS $$
BEGIN
    IF CURRENT_TIMESTAMP > NEW.endlifespanversion THEN
        RAISE EXCEPTION 'If you change record endlifespanversion must be greater than today';
...
ON soilprofile
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilprofile_error();


CREATE OR REPLACE FUNCTION validate_wrbversion_sp()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON soilprofile
FOR EACH ROW
EXECUTE FUNCTION validate_wrbversion_sp();
...
(
    ...
    othersoilname UUID,
    FOREIGN KEY (othersoilname) REFERENCES soilprofile(guidkey) ON DELETE CASCADE ON UPDATE CASCADE
);

COMMENT ON TABLE othersoilnametype IS 'An identification of the soil profile according to a specific classification scheme.';
-- Add Comment Column
COMMENT ON COLUMN othersoilnametype.id IS 'Primary Key of the Table';
COMMENT ON COLUMN othersoilnametype.othersoilname_type IS 'Name of the soil profile according to a specific classification scheme.';
COMMENT ON COLUMN othersoilnametype.othersoilname_class IS 'Specific classification scheme.';
COMMENT ON COLUMN othersoilnametype.isoriginalclassification IS 'Boolean value to indicate whether the specified classification scheme was the original classification scheme to describe the profile.';
COMMENT ON COLUMN othersoilnametype.othersoilname IS 'Foreign key to the SoilProfile table, guidkey field.';



-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_soil_name()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON othersoilnametype
FOR EACH ROW
EXECUTE FUNCTION check_soil_name();
...
COMMENT ON TABLE isderivedfrom IS 'Link between Derived Soil Profiles and Observed Soil Profiles.';
-- Add Comment Column
--COMMENT ON COLUMN isderivedfrom.id IS 'Primary Key of the Table';
COMMENT ON COLUMN isderivedfrom.base_id IS 'Foreign key to the SoilProfile table, guidkey field. - Observed Soil Profile';
COMMENT ON COLUMN isderivedfrom.related_id IS 'Foreign key to the SoilProfile table, guidkey field. - Derived Soil Profile';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_isderived()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON isderivedfrom
FOR EACH ROW
EXECUTE FUNCTION check_isderived();

CREATE OR REPLACE FUNCTION check_isobserved()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON isderivedfrom
FOR EACH ROW
EXECUTE FUNCTION check_isobserved();
...
COMMENT ON TABLE soilbody IS 'Part of the soil cover that is delineated and that is homogeneous with regard to certain soil properties and/or spatial patterns.';
-- Add Comment Column
--COMMENT ON COLUMN soilbody.id IS 'Primary Key of the Table';
COMMENT ON COLUMN soilbody.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN soilbody.inspireid_localid IS 'A local identifier, assigned by the data provider. The local identifier is unique within the namespace, that is no other spatial object carries the same unique identifier.';
COMMENT ON COLUMN soilbody.inspireid_namespace IS 'Namespace uniquely identifying the data source of the spatial object.';
COMMENT ON COLUMN soilbody.inspireid_versionid IS 'The identifier of the particular version of the spatial object, with a maximum length of 25 characters. If the specification of a spatial object type with an external object identifier includes life-cycle information, the version identifier is used to distinguish between the different versions of a spatial object. Within the set of all versions of a spatial object, the version identifier is unique.';
COMMENT ON COLUMN soilbody.beginlifespanversion IS 'Date and time at which this version of the spatial object was inserted or changed in the spatial data set.';
COMMENT ON COLUMN soilbody.endlifespanversion IS 'Date and time at which this version of the spatial object was superseded or retired in the spatial data set.';
COMMENT ON COLUMN soilbody.soilbodylabel IS 'Label to identify the soil body according to the specified reference framework (metadata).';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_valid_version_soilbody()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.beginlifespanversion > NEW.endlifespanversion THEN
...
BEFORE INSERT OR UPDATE ON soilbody
FOR EACH ROW
EXECUTE FUNCTION check_valid_version_soilbody();

CREATE OR REPLACE FUNCTION update_begin_today_soilbody()
RETURNS TRIGGER AS $$
BEGIN
    IF CURRENT_TIMESTAMP < NEW.endlifespanversion OR NEW.endlifespanversion IS NULL THEN
        NEW.beginlifespanversion := CURRENT_TIMESTAMP;
...
ON soilbody
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilbody();


CREATE OR REPLACE FUNCTION update_begin_today_soilbody_error()
RETURNS TRIGGER AS $$
BEGIN
    IF CURRENT_TIMESTAMP > NEW.endlifespanversion THEN
        RAISE EXCEPTION 'If you change record endlifespanversion must be greater than today';
...
ON soilbody
FOR EACH ROW
EXECUTE FUNCTION update_begin_today_soilbody_error();
...
COMMENT ON TABLE soilbody_geom IS 'Shape of a Soil Body.';
-- Add Comment Column
COMMENT ON COLUMN soilbody_geom.id IS 'Primary Key of the Table';
COMMENT ON COLUMN soilbody_geom.idsoilbody IS 'Foreign key to the SoilBody table, guidkey field.';
COMMENT ON COLUMN soilbody_geom.geom IS 'Geometry';
...
COMMENT ON TABLE derivedprofilepresenceinsoilbody IS 'Indicates the percentages (lower and upper boundary) that the derived profile takes part in the Soil body.';
-- Add Comment Column
--COMMENT ON COLUMN derivedprofilepresenceinsoilbody.id IS 'Primary Key of the Table';
COMMENT ON COLUMN derivedprofilepresenceinsoilbody.idsoilbody IS 'Foreign key to the SoilBody table, guidkey field.';
COMMENT ON COLUMN derivedprofilepresenceinsoilbody.idsoilprofile IS 'Foreign key to the SoilProfile table, guidkey field.';
COMMENT ON COLUMN derivedprofilepresenceinsoilbody.lowervalue IS 'Upper value.';
COMMENT ON COLUMN derivedprofilepresenceinsoilbody.uppervalue IS 'Lower value'; 


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_lowervaluesum()
RETURNS TRIGGER AS $$
DECLARE
...
BEFORE INSERT OR UPDATE ON derivedprofilepresenceinsoilbody
FOR EACH ROW
EXECUTE FUNCTION check_lowervaluesum();

CREATE OR REPLACE FUNCTION check_isderived_soilbody()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON derivedprofilepresenceinsoilbody
FOR EACH ROW
EXECUTE FUNCTION check_isderived_soilbody();
...
COMMENT ON TABLE soilderivedobject IS 'A spatial object type for representing spatial objects with soil-related property derived from one or more soil and possibly other non soil properties.';
-- Add Comment Column
-- COMMENT ON COLUMN soilderivedobject.id IS 'Primary Key of the Table';
COMMENT ON COLUMN soilderivedobject.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN soilderivedobject.inspireid_localid IS 'A local identifier, assigned by the data provider. The local identifier is unique within the namespace, that is no other spatial object carries the same unique identifier.';
COMMENT ON COLUMN soilderivedobject.inspireid_namespace IS 'Namespace uniquely identifying the data source of the spatial object.';
COMMENT ON COLUMN soilderivedobject.inspireid_versionid IS 'The identifier of the particular version of the spatial object, with a maximum length of 25 characters. If the specification of a spatial object type with an external object identifier includes life-cycle information, the version identifier is used to distinguish between the different versions of a spatial object. Within the set of all versions of a spatial object, the version identifier is unique.';
COMMENT ON COLUMN soilderivedobject.accessuri IS 'SoilDerivedObject URI';
COMMENT ON COLUMN soilderivedobject.geometry IS 'Geometry'; 
...
COMMENT ON TABLE isbasedonobservedsoilprofile IS 'Table of Link to an observed soil profile on whose properties the derived value is based.';
-- Add Comment Column
-- COMMENT ON COLUMN isbasedonobservedsoilprofile.id IS 'Primary Key of the Table';
COMMENT ON COLUMN isbasedonobservedsoilprofile.idsoilderivedobject IS 'Foreign key to the SoilDerivedObject table, guidkey field.';
COMMENT ON COLUMN isbasedonobservedsoilprofile.idsoilprofile IS 'Foreign key to the SoilProfile table, guidkey field.';

-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_isobserved_dobj()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON isbasedonobservedsoilprofile
FOR EACH ROW
EXECUTE FUNCTION check_isobserved_dobj();
...
COMMENT ON TABLE isbasedonsoilbody IS 'Table of Link to a soil body on whose properties the derived value is based.';
-- Add Comment Column
-- COMMENT ON COLUMN isbasedonsoilbody.id IS 'Primary Key of the Table';
COMMENT ON COLUMN isbasedonsoilbody.idsoilderivedobject IS 'Foreign key to the SoilDerivedObject table, guidkey field.';
COMMENT ON COLUMN isbasedonsoilbody.idsoilbody IS 'Foreign key to the SoilBody table, guidkey field.';
...
COMMENT ON TABLE isbasedonsoilderivedobject IS 'Table of Link to a soil derived object on whose properties the derived value is based.';
-- Add Comment Column
-- COMMENT ON COLUMN isbasedonsoilderivedobject.id IS 'Primary Key of the Table';
COMMENT ON COLUMN isbasedonsoilderivedobject.base_id IS 'Foreign key to the SoilDerivedObject table, guidkey field. - Base SoilDerivedObject';
COMMENT ON COLUMN isbasedonsoilderivedobject.related_id IS 'Foreign key to the SoilDerivedObject table, guidkey field. - Derived SoilDerivedObject';
...
COMMENT ON TABLE profileelement IS 'An abstract spatial object type grouping soil layers and / or horizons for functional/operational aims.';
-- Add Comment Column
-- COMMENT ON COLUMN profileelement.id IS 'Primary Key of the Table';
COMMENT ON COLUMN profileelement.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN profileelement.inspireid_localid IS 'A local identifier, assigned by the data provider. The local identifier is unique within the namespace, that is no other spatial object carries the same unique identifier.';
COMMENT ON COLUMN profileelement.inspireid_namespace IS 'Namespace uniquely identifying the data source of the spatial object.';
COMMENT ON COLUMN profileelement.inspireid_versionid IS 'The identifier of the particular version of the spatial object, with a maximum length of 25 characters. If the specification of a spatial object type with an external object identifier includes life-cycle information, the version identifier is used to distinguish between the different versions of a spatial object. Within the set of all versions of a spatial object, the version identifier is unique.';
COMMENT ON COLUMN profileelement.profileelementdepthrange_uppervalue IS 'Upper depth of the profile element (layer or horizon) measured from the surface of a soil profile (in cm)';
COMMENT ON COLUMN profileelement.profileelementdepthrange_lowervalue IS 'Lower depth of the profile element (layer or horizon) measured from the surface of a soil profile (in cm)';
COMMENT ON COLUMN profileelement.beginlifespanversion IS 'Date and time at which this version of the spatial object was inserted or changed in the spatial data set.';
COMMENT ON COLUMN profileelement.endlifespanversion IS 'Date and time at which this version of the spatial object was superseded or retired in the spatial data set.';
COMMENT ON COLUMN profileelement.layertype IS 'Assignation of a layer according to the concept that fits its kind.';
COMMENT ON COLUMN profileelement.layerrocktype IS 'Type of the material in which the layer developed.';
COMMENT ON COLUMN profileelement.layergenesisprocess IS 'Last non-pedogenic process (geologic or anthropogenic) that coined the material composition and internal structure of the layer.';
COMMENT ON COLUMN profileelement.layergenesisenviroment IS 'Setting in which the last non-pedogenic process (geologic or anthropogenic) that coined the material composition and internal structure of the layer took place.';
COMMENT ON COLUMN profileelement.layergenesisprocessstate IS 'Indication whether the process specified in layerGenesisProcess is on-going or seized in the past.';
COMMENT ON COLUMN profileelement.profileelementtype IS 'Boolean value to indicate whether the record is of Horizon or Layer type';
COMMENT ON COLUMN profileelement.ispartof IS 'Foreign key to the SoilProfile table, guidkey field.';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_valid_version_profileelement()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.beginlifespanversion > NEW.endlifespanversion THEN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_valid_version_profileelement();

CREATE OR REPLACE FUNCTION check_valid_deepprofileelement()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_valid_deepprofileelement();

CREATE OR REPLACE FUNCTION check_geogenic_fields_not_null()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.layertype = 'http://inspire.ec.europa.eu/codelist/LayerTypeValue/geogenic' THEN
        IF NEW.layerrocktype IS NULL THEN
            RAISE EXCEPTION 'layerrocktype cannot be NULL when LayerTypeValue is "geogenic".';
        END IF;
        IF NEW.layergenesisprocess IS NULL THEN
            RAISE EXCEPTION 'layergenesisprocess cannot be NULL when LayerTypeValue is "geogenic".';
        END IF;
        IF NEW.layergenesisenviroment IS NULL THEN
            RAISE EXCEPTION 'layergenesisenviroment cannot be NULL when LayerTypeValue is "geogenic".';
        END IF;
        IF NEW.layergenesisprocessstate IS NULL THEN
            RAISE EXCEPTION 'layergenesisprocessstate cannot be NULL when LayerTypeValue is "geogenic".';
        END IF;
    END IF;
    RETURN NEW;
END;
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_geogenic_fields_not_null();

CREATE OR REPLACE FUNCTION check_horizon_fields()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_horizon_fields();


CREATE OR REPLACE FUNCTION check_layertype()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_layertype();


CREATE OR REPLACE FUNCTION check_layergenesisenviroment()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_layergenesisenviroment();

CREATE OR REPLACE FUNCTION check_layergenesisprocess()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_layergenesisprocess();


CREATE OR REPLACE FUNCTION check_layergenesisprocessstate()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_layergenesisprocessstate();


CREATE OR REPLACE FUNCTION check_layerrocktype()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON profileelement
FOR EACH ROW
EXECUTE FUNCTION check_layerrocktype();


CREATE OR REPLACE FUNCTION check_depth_range_function() 
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON  profileelement
FOR EACH ROW
EXECUTE FUNCTION check_depth_range_function();


CREATE OR REPLACE FUNCTION u_begin_today_profileelement()
RETURNS TRIGGER AS
$$
BEGIN
    IF (current_timestamp < NEW.endlifespanversion OR NEW.endlifespanversion IS NULL) THEN
...
ON profileelement
FOR EACH ROW
EXECUTE FUNCTION u_begin_today_profileelement();

CREATE OR REPLACE FUNCTION u_begin_today_profileelement_error()
RETURNS TRIGGER AS
$$
BEGIN
    IF (current_timestamp > NEW.endlifespanversion OR NEW.endlifespanversion IS NULL) THEN
...
ON profileelement
FOR EACH ROW
EXECUTE FUNCTION u_begin_today_profileelement_error();
...
COMMENT ON TABLE particlesizefractiontype IS 'Share of the soil that is composed of mineral soil particles of the size within the size range specified.';
-- Add Comment Column
COMMENT ON COLUMN particlesizefractiontype.id IS 'Primary Key of the Table';
COMMENT ON COLUMN particlesizefractiontype.fractioncontent IS 'Percentage of the defined fraction.';
COMMENT ON COLUMN particlesizefractiontype.pariclesize_min IS 'Upper limit of the particle size of the defined fraction (expressed in µm)';
COMMENT ON COLUMN particlesizefractiontype.pariclesize_max IS 'Lower limit of the particle size of the defined fraction (expressed in µm)';
COMMENT ON COLUMN particlesizefractiontype.idprofileelement IS 'Foreign key to the ProfileElement table, guidkey field.';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_fraction_sum()
RETURNS TRIGGER AS
$$
BEGIN
    -- Controlla se la somma supera 100
    IF (
        SELECT SUM(fractioncontent) + NEW.fractioncontent
        FROM particlesizefractiontype
        WHERE idprofileelement = NEW.idprofileelement
    ) > 100 THEN
        -- Genera un errore se la somma supera 100
        RAISE EXCEPTION 'La somma di fractioncontent per idprofileelement non può superare 100';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER check_fraction_sum
BEFORE INSERT ON  particlesizefractiontype
FOR EACH ROW
EXECUTE FUNCTION check_fraction_sum();



CREATE OR REPLACE FUNCTION u_check_fraction_sum() RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION u_check_fraction_sum();
...
COMMENT ON TABLE faohorizonnotationtype IS 'A classification of a horizon according to the Horizon classification system specified in Guidelines for soil description, 4th edition, Food and Agriculture Organization of the United Nations, Rome, 2006.';
-- Add Comment Column
COMMENT ON COLUMN faohorizonnotationtype.id IS 'Primary Key of the Table';
COMMENT ON COLUMN faohorizonnotationtype.faohorizondiscontinuity IS 'Number used to indicate a discontinuity in the horizon notation.';
COMMENT ON COLUMN faohorizonnotationtype.faohorizonmaster IS 'Symbol of the master part of the horizon notation.';
COMMENT ON COLUMN faohorizonnotationtype.faohorizonsubordinate IS 'Designations of subordinate distinctions and features within the master horizons and layers are based on profile characteristics observable in the field and are applied during the description of the soil at the site.';
COMMENT ON COLUMN faohorizonnotationtype.faohorizonverical IS 'Order number of the vertical subdivision in the horizon notation.';
COMMENT ON COLUMN faohorizonnotationtype.faoprime IS 'A prime and double prime may be used to connotate master horizon symbol of the lower of two respectively three horizons having identical Arabic-numeral prefixes and letter combinations.';
COMMENT ON COLUMN faohorizonnotationtype.isoriginalclassification IS 'Boolean value to indicate whether the FAO horizon notation was the original notation to describe the horizon.';
COMMENT ON COLUMN faohorizonnotationtype.idprofileelement IS 'Foreign key to the ProfileElement table, guidkey field.';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_faoprofileelementtype()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON faohorizonnotationtype
FOR EACH ROW
EXECUTE FUNCTION check_faoprofileelementtype();


CREATE OR REPLACE FUNCTION check_faohorizonmaster()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.faohorizonmaster IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM codelist WHERE id = NEW.faohorizonmaster AND collection = 'FAOHorizonMasterValue') THEN
...
CREATE TRIGGER faohorizonmaster
BEFORE INSERT OR UPDATE ON faohorizonnotationtype
FOR EACH ROW
EXECUTE FUNCTION check_faohorizonmaster();


CREATE OR REPLACE FUNCTION check_faohorizonsubordinate()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.faohorizonsubordinate IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM codelist WHERE id = NEW.faohorizonsubordinate AND collection = 'FAOHorizonSubordinateValue') THEN
...
CREATE TRIGGER faohorizonsubordinate
BEFORE INSERT OR UPDATE ON faohorizonnotationtype
FOR EACH ROW
EXECUTE FUNCTION check_faohorizonsubordinate();

CREATE OR REPLACE FUNCTION check_faoprime()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON faohorizonnotationtype
FOR EACH ROW
EXECUTE FUNCTION check_faoprime();
...
COMMENT ON TABLE otherhorizonnotationtype IS 'classification of a horizon according to a specific classification system.';
-- Add Comment Column
-- COMMENT ON COLUMN otherhorizonnotationtype.id IS 'Primary Key of the Table';
COMMENT ON COLUMN otherhorizonnotationtype.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN otherhorizonnotationtype.horizonnotation IS 'Notation characterizing the soil horizon according to a specified classification system.';
COMMENT ON COLUMN otherhorizonnotationtype.diagnostichorizon IS 'Codelist wrbdiagnostichorizonvalue';
COMMENT ON COLUMN otherhorizonnotationtype.isoriginalclassification IS 'Boolean value to indicate whether the specified horizon notation system was the original notation system to describe the horizon.';
COMMENT ON COLUMN otherhorizonnotationtype.otherhorizonnotation IS '';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_horizonnotation_insert()
RETURNS TRIGGER AS
$$
BEGIN
...
BEFORE INSERT OR UPDATE ON otherhorizonnotationtype
FOR EACH ROW
EXECUTE FUNCTION check_horizonnotation_insert();


CREATE OR REPLACE FUNCTION check_diagnostichorizon_insert()
RETURNS TRIGGER AS
$$
BEGIN
...
BEFORE INSERT OR UPDATE ON  otherhorizonnotationtype
FOR EACH ROW
EXECUTE FUNCTION check_diagnostichorizon_insert();
...
COMMENT ON TABLE otherhorizon_profileelement IS 'Table of Link between OtherHorizonNotationType and ProfilEelement.';
-- Add Comment Column
--COMMENT ON COLUMN otherhorizon_profileelement.id IS 'Primary Key of the Table';
COMMENT ON COLUMN otherhorizon_profileelement.idprofileelement IS 'Foreign key to the ProfileElement table, guidkey field.';
COMMENT ON COLUMN otherhorizon_profileelement.idotherhorizonnotationtype IS 'Foreign key to the OtherhorizonNotationType table, guidkey field.';

-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_otherhorizon_profileelement_profileelementtype()
RETURNS TRIGGER AS $$
BEGIN
    IF (SELECT profileelementtype FROM profileelement WHERE guidkey = NEW.idprofileelement) = 1 THEN
...
BEFORE INSERT OR UPDATE ON otherhorizon_profileelement
FOR EACH ROW
EXECUTE FUNCTION check_otherhorizon_profileelement_profileelementtype();
...
COMMENT ON TABLE wrbqualifiergrouptype IS 'A data type to define the group of a qualifier and its possible specifier(s), its place and position with regard to the World Reference Base (WRB) Reference Soil Group (RSG) it belongs to according to World reference base for soil resources 2006, first update 2007, World Soil Resources Reports No. 103, Food and Agriculture Organization of the United Nations, Rome, 2007';
-- Add Comment Column
--COMMENT ON COLUMN wrbqualifiergrouptype.id IS 'Primary Key of the Table';
COMMENT ON COLUMN wrbqualifiergrouptype.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN wrbqualifiergrouptype.qualifierplace IS 'Attribute to indicate the placement of the Qualifier with regard to the WRB reference soil group (RSG). The placement can be in front of the RSG i.e. prefix or it can be behind the RSG i.e. suffix.';
COMMENT ON COLUMN wrbqualifiergrouptype.wrbqualifier IS 'Name element of WRB, 2nd level of classification';
COMMENT ON COLUMN wrbqualifiergrouptype.wrbspecifier_1 IS 'Code that indicates the degree of expression of a qualifier or the depth range of which the qualifier applies.';
COMMENT ON COLUMN wrbqualifiergrouptype.wrbspecifier_2 IS 'Code that indicates the degree of expression of a qualifier or the depth range of which the qualifier applies.';
COMMENT ON COLUMN wrbqualifiergrouptype.wrbversion IS 'Indicates the WRB classification version.';
...
CREATE OR REPLACE FUNCTION check_qualifierplace_wrbqualifiergrouptype()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON wrbqualifiergrouptype
FOR EACH ROW
EXECUTE FUNCTION check_qualifierplace_wrbqualifiergrouptype();
...
CREATE OR REPLACE FUNCTION validate_wrbversion_gt()
RETURNS TRIGGER AS $$
BEGIN
    IF (NEW.wrbversion NOT IN (SELECT id FROM codelist WHERE collection = 'wrbversion') AND NEW.wrbversion IS NOT NULL) THEN
...
BEFORE INSERT OR UPDATE ON wrbqualifiergrouptype
FOR EACH ROW
EXECUTE FUNCTION validate_wrbversion_gt();


CREATE OR REPLACE FUNCTION validate_unique_wrbqualifiergrouptype()
RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (
        SELECT 1
        ...
        WHERE 
            (NEW.wrbversion IS NOT NULL AND NEW.wrbversion = wrbqualifiergrouptype.wrbversion OR NEW.wrbversion IS NULL AND wrbqualifiergrouptype.wrbversion IS NULL)
            AND 
            (NEW.qualifierplace IS NOT NULL AND NEW.qualifierplace = wrbqualifiergrouptype.qualifierplace OR NEW.qualifierplace IS NULL AND wrbqualifiergrouptype.qualifierplace IS NULL)
            AND 
            (NEW.wrbqualifier IS NOT NULL AND NEW.wrbqualifier = wrbqualifiergrouptype.wrbqualifier OR NEW.wrbqualifier IS NULL AND wrbqualifiergrouptype.wrbqualifier IS NULL)
            AND 
            (NEW.wrbspecifier_1 IS NOT NULL AND NEW.wrbspecifier_1 = wrbqualifiergrouptype.wrbspecifier_1 OR NEW.wrbspecifier_1 IS NULL AND wrbqualifiergrouptype.wrbspecifier_1 IS NULL)
            AND 
            (NEW.wrbspecifier_2 IS NOT NULL AND NEW.wrbspecifier_2 = wrbqualifiergrouptype.wrbspecifier_2 OR NEW.wrbspecifier_2 IS NULL AND wrbqualifiergrouptype.wrbspecifier_2 IS NULL)
    ) THEN
...
BEFORE INSERT OR UPDATE ON wrbqualifiergrouptype
FOR EACH ROW
EXECUTE FUNCTION validate_unique_wrbqualifiergrouptype();


CREATE OR REPLACE FUNCTION check_specifiers_not_equal()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON wrbqualifiergrouptype
FOR EACH ROW
EXECUTE FUNCTION check_specifiers_not_equal();
...
COMMENT ON TABLE wrbqualifiergroup_profile IS 'Table of Link between WrbQualifierGroupType and SoilProfile.';
-- Add Comment Column
-- COMMENT ON COLUMN wrbqualifiergroup_profile.id IS 'Primary Key of the Table';
COMMENT ON COLUMN wrbqualifiergroup_profile.idsoilprofile IS 'Foreign key to the SaoilProfile table, guidkey field.';
COMMENT ON COLUMN wrbqualifiergroup_profile.idwrbqualifiergrouptype IS 'Foreign key to the WrbqualifierGroupType table, guidkey field.';
COMMENT ON COLUMN wrbqualifiergroup_profile.qualifierposition IS 'Number to indicate the position of a qualifier with regard to the WRB reference soil group (RSG) it belongs to and with regard to its placement to that (RSG) i.e. as a prefix or a suffix.';

-- Trigger and Function -------------------------------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION check_wrbversion_match()
RETURNS TRIGGER AS $$
BEGIN
    IF (
...
BEFORE INSERT OR UPDATE ON wrbqualifiergroup_profile
FOR EACH ROW
EXECUTE FUNCTION check_wrbversion_match();


CREATE OR REPLACE FUNCTION check_qualifier_position_unique()
RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (
        SELECT 1
...
BEFORE INSERT OR UPDATE ON wrbqualifiergroup_profile
FOR EACH ROW
EXECUTE FUNCTION check_qualifier_position_unique();
...
COMMENT ON TABLE thing IS 'Object containing the sensor(s) producing the datae.';
-- Add Comment Column
-- COMMENT ON COLUMN thing.id IS 'Primary Key of the Table';
COMMENT ON COLUMN thing.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN thing.name IS 'Name of the object containing the sensor(s) producing the data';
COMMENT ON COLUMN thing.description IS 'Description of the object contains the sensor(s) that produce the data';
COMMENT ON COLUMN thing.properties IS 'File containing object properties';
...
COMMENT ON TABLE datastreamcollection IS 'Collection of Dta Stream.';
-- Add Comment Column
-- COMMENT ON COLUMN datastreamcollection.id IS 'Primary Key of the Table';
COMMENT ON COLUMN datastreamcollection.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN datastreamcollection.name IS 'Name of the data stream ';
COMMENT ON COLUMN datastreamcollection.description IS 'Description of the data stream ';
COMMENT ON COLUMN datastreamcollection.observedarea IS 'Data stream observation area';
COMMENT ON COLUMN datastreamcollection.beginphenomenontime IS 'Start of PhenomenonTime, which describes the time at which the result applies to the property of the feature of interest';
COMMENT ON COLUMN datastreamcollection.endphenomenontime IS 'End of PhenomenonTime, which describes the time at which the result applies to the property of the feature of interest';
COMMENT ON COLUMN datastreamcollection.beginresulttime IS 'Start of the resultTime, which describes when the result became available, typically when the procedure associated with the observation was completed';
COMMENT ON COLUMN datastreamcollection.endresulttime IS 'End of resultTime, which describes when the result became available, typically when the procedure associated with the observation was completed';
COMMENT ON COLUMN datastreamcollection.properties IS 'File containing object properties';
COMMENT ON COLUMN datastreamcollection.idthing IS 'Foreign key to the Thing table, guidkey field.';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_valid_phenomenon_time()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON datastreamcollection
FOR EACH ROW
EXECUTE FUNCTION check_valid_phenomenon_time();


CREATE OR REPLACE FUNCTION check_valid_result_time()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON datastreamcollection
FOR EACH ROW
EXECUTE FUNCTION check_valid_result_time();
...
COMMENT ON TABLE unitofmeasure IS 'Any of the systems devised to measure some physical quantity such distance or area or a system devised to measure such things as the passage of time.';
-- Add Comment Column
-- COMMENT ON COLUMN unitofmeasure.id IS 'Primary Key of the Table';
COMMENT ON COLUMN unitofmeasure.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN unitofmeasure.uomname IS 'The name(s) of a particular unit of measure.';
COMMENT ON COLUMN unitofmeasure.uomsymbol IS 'The symbol used for this unit of measure, such at "ft" for feet, or "m" for meter.';
COMMENT ON COLUMN unitofmeasure.measuretype IS 'Type of Measure';
COMMENT ON COLUMN unitofmeasure.namestandardunit IS 'Name of the standard units to which this unit of measure can be directly converted.';
COMMENT ON COLUMN unitofmeasure.scaletostandardunit IS 'Multiplicative factor for conversion of this unit of measure to the standard one (often the ISO standard unit).';
COMMENT ON COLUMN unitofmeasure.offsettostandardunit IS 'Offset(ToStandardUnit) can be used to make the conversion from X to S by';
COMMENT ON COLUMN unitofmeasure.formula IS 'An algebraic formula (probably in some programming language) converting this unit of measure (represented in the formula by its uomSymbol) to the ISO standard (represented by its symbol.';
...
COMMENT ON TABLE observableproperty IS 'Represents a single observable property e.g. temperature.';
-- Add Comment Column
-- COMMENT ON COLUMN observableproperty.id IS 'Primary Key of the Table';
COMMENT ON COLUMN observableproperty.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN observableproperty.name IS 'Name of the Observable Property';
COMMENT ON COLUMN observableproperty.definition IS 'Definition of the Observable Property';
COMMENT ON COLUMN observableproperty.description IS 'Description of the Observable Property';
COMMENT ON COLUMN observableproperty.foi IS 'The Feature of Interest to which the Observable Property belongs';
COMMENT ON COLUMN observableproperty.phenomenontype IS 'Type of phenomenon to which the Observable Property belongs';
COMMENT ON COLUMN observableproperty.basephenomenon IS 'The phenomenon that the Observable Property description builds upon.';
COMMENT ON COLUMN observableproperty.domain_min IS 'Minimum limit of the domain of values ​​that the Observable Property can take on';
COMMENT ON COLUMN observableproperty.domain_max IS 'Maximum limit of the domain of values ​​that the Observable Property can take on';
COMMENT ON COLUMN observableproperty.domain_typeofvalue IS 'Whether the value is numeric or encoded';
COMMENT ON COLUMN observableproperty.domain_code IS 'Encoding in case the value is encoded';
COMMENT ON COLUMN observableproperty.iduom IS 'Foreign key to the UnitOfMeasure table, guidkey field.';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_foi()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER foi
BEFORE INSERT OR UPDATE ON observableproperty
FOR EACH ROW
EXECUTE FUNCTION check_foi();


CREATE OR REPLACE FUNCTION check_phenomenon_type()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON observableproperty
FOR EACH ROW
EXECUTE FUNCTION check_phenomenon_type();


CREATE OR REPLACE FUNCTION check_base_phenomenon()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON observableproperty
FOR EACH ROW
EXECUTE FUNCTION check_base_phenomenon();

CREATE OR REPLACE FUNCTION check_domain()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION check_domain_value()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON observableproperty
FOR EACH ROW
EXECUTE FUNCTION check_domain_value();
...
COMMENT ON TABLE sensor IS 'Represents a Sensor Object';
-- Add Comment Column
-- COMMENT ON COLUMN sensor.id IS 'Primary Key of the Table';
COMMENT ON COLUMN sensor.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN sensor.name IS 'Code of the Sensor';
COMMENT ON COLUMN sensor.description IS 'Description of the Sensor';
...
COMMENT ON TABLE relatedparty IS 'An organisation or a person with a role related to a resource.';
-- Add Comment Column
-- COMMENT ON COLUMN relatedparty.id IS 'Primary Key of the Table';
COMMENT ON COLUMN relatedparty.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN relatedparty.individualname IS 'Name of the related person.';
COMMENT ON COLUMN relatedparty.organizationname IS 'Name of the related person.';
COMMENT ON COLUMN relatedparty.positionname IS 'Position of the party in relation to a resource, such as head of department.';
COMMENT ON COLUMN relatedparty.address IS 'An address provided as free text.';
COMMENT ON COLUMN relatedparty.contactinstructions IS 'Supplementary instructions on how or when to contact an individual or organisation.';
COMMENT ON COLUMN relatedparty.electronicmailaddress IS 'An electronic mailbox address of the organisation or individual.';
COMMENT ON COLUMN relatedparty.hoursofservice IS 'Periods of time when the organisation or individual can be contacted.';
COMMENT ON COLUMN relatedparty.telephonefacsimile IS 'Number of a facsimile machine of the organisation or individual.';
COMMENT ON COLUMN relatedparty.telephonevoice IS 'Telephone number of the organisation or individual.';
COMMENT ON COLUMN relatedparty.website IS 'Pages provided on the World Wide Web by the organisation or individual.';
COMMENT ON COLUMN relatedparty.role IS 'Role(s) of the party in relation to a resource, such as owner.';
...
CREATE TRIGGER role
BEFORE INSERT OR UPDATE ON relatedparty
FOR EACH ROW
...
COMMENT ON TABLE documentcitation IS 'Citation for the purposes of unambiguously referencing a document.';
-- Add Comment Column
-- COMMENT ON COLUMN documentcitation.id IS 'Primary Key of the Table';
COMMENT ON COLUMN documentcitation.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN documentcitation.name IS 'Name of the document.';
COMMENT ON COLUMN documentcitation.shortname IS 'Short name or alternative title of the document.';
COMMENT ON COLUMN documentcitation.date IS 'Date of creation, publication or revision of the document.';
COMMENT ON COLUMN documentcitation.link IS 'Link to an online version of the document.';
COMMENT ON COLUMN documentcitation.specificreference IS 'Reference to a specific part of the document.';
...
COMMENT ON TABLE process IS 'Description of an observation process';
-- Add Comment Column
-- COMMENT ON COLUMN process.id IS 'Primary Key of the Table';
COMMENT ON COLUMN process.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN process.inspireid_localid IS 'A local identifier, assigned by the data provider. The local identifier is unique within the namespace, that is no other spatial object carries the same unique identifier.';
COMMENT ON COLUMN process.inspireid_namespace IS 'Namespace uniquely identifying the data source of the spatial object.';
COMMENT ON COLUMN process.inspireid_versionid IS 'The identifier of the particular version of the spatial object, with a maximum length of 25 characters. If the specification of a spatial object type with an external object identifier includes life-cycle information, the version identifier is used to distinguish between the different versions of a spatial object. Within the set of all versions of a spatial object, the version identifier is unique.';
COMMENT ON COLUMN process.name IS 'Name of the Process';
COMMENT ON COLUMN process.description IS 'Description of the Proces';
COMMENT ON COLUMN process.type IS 'Type of process.';
COMMENT ON COLUMN process.idrelatedparty1 IS 'First Foreign key to the RelatedParty table, guidkey field.';
COMMENT ON COLUMN process.idrelatedparty2 IS 'Second Foreign key to the RelatedParty table, guidkey field.';
COMMENT ON COLUMN process.iddocumentcitation1 IS 'FirstForeign key to the DocumentCitation table, guidkey field.';
COMMENT ON COLUMN process.iddocumentcitation2 IS 'Second Foreign key to the DocumentCitation table, guidkey field.';
...
CREATE TABLE observableproperty_process (
    ...
    FOREIGN KEY (idprocess)
      REFERENCES process(guidkey) ON DELETE CASCADE ON UPDATE CASCADE,

    FOREIGN KEY (idobservedproperty)
      REFERENCES observableproperty(guidkey) ON DELETE CASCADE ON UPDATE CASCADE
);


COMMENT ON TABLE observableproperty_process IS 'Table of Link between Process and ObservableProperty.';
-- Add Comment Column
COMMENT ON COLUMN observableproperty_process.idprocess IS 'Foreign key to the Process table, guidkey field. - Observed Soil Profile';
COMMENT ON COLUMN observableproperty_process.idobservedproperty IS 'Foreign key to the ObservablePropery table, guidkey field. - Derived Soil Profile';
...
COMMENT ON TABLE datastream IS 'Stream of Data.';
-- Add Comment Column
-- COMMENT ON COLUMN datastream.id IS 'Primary Key of the Table';
COMMENT ON COLUMN datastream.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN datastream.idsoilsite IS 'Foreign key to the Process table, guidkey field.';
COMMENT ON COLUMN datastream.idsoilprofile IS 'Foreign key to the SoilProfile table, guidkey field.';
COMMENT ON COLUMN datastream.idsoilderivedobject IS 'Foreign key to the SoilDerivedObject table, guidkey field.';
COMMENT ON COLUMN datastream.idprofileelement IS 'Foreign key to the ProfileElement table, guidkey field.';
COMMENT ON COLUMN datastream.iddatastreamcollection IS 'Foreign key to the DatastreamCollection table, guidkey field.';
COMMENT ON COLUMN datastream.idprocess IS 'Foreign key to the Process table, guidkey field.';
COMMENT ON COLUMN datastream.idobservedproperty IS 'Foreign key to the ObservablePropery table, guidkey field.';
COMMENT ON COLUMN datastream.idsensor IS 'Foreign key to the Datastream table, guidkey field.';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_soilprofile_obspro()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON datastream
FOR EACH ROW
EXECUTE FUNCTION check_soilprofile_obspro();

CREATE OR REPLACE FUNCTION check_soilsite_obspro()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON datastream
FOR EACH ROW
EXECUTE FUNCTION check_soilsite_obspro();

CREATE OR REPLACE FUNCTION check_profileelement_obspro()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON datastream
FOR EACH ROW
EXECUTE FUNCTION check_profileelement_obspro();

CREATE OR REPLACE FUNCTION check_soilderivedobject_obspro()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON datastream
FOR EACH ROW
EXECUTE FUNCTION check_soilderivedobject_obspro();
...
(
    ...
    FOREIGN KEY (iddatastream)
      REFERENCES datastream(guidkey) ON DELETE CASCADE ON UPDATE CASCADE
);


COMMENT ON TABLE observation IS 'An observation is an act that results in the estimation of the value of a feature property, and involves application of a specified procedure, such as a sensor, instrument, algorithm or process chain.';
-- Add Comment Column
-- COMMENT ON COLUMN observation.id IS 'Primary Key of the Table';
COMMENT ON COLUMN observation.guidkey IS 'Universally unique identifier';
COMMENT ON COLUMN observation.phenomenontime IS 'Describe the time that the result applies to the property of the feature-of-interest';
COMMENT ON COLUMN observation.resulttime IS 'Describe the time when the result became available, typically when the procedure associated with the observation was completed For some observations this is identical to the phenomenonTime.';
COMMENT ON COLUMN observation.validtime IS 'Describe the time period during which the result is intended to be used.';
COMMENT ON COLUMN observation.resultquality IS 'Describe the quality of the result';
COMMENT ON COLUMN observation.result_value IS 'Numeric Value of the Observation';
COMMENT ON COLUMN observation.result_uri IS 'Coded Value of the Observation';;
COMMENT ON COLUMN observation.iddatastream IS 'Foreign key to the Datastream table, guidkey field.';


-- Trigger and Function -------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION check_validperiod_observation()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION check_validperiod_observation();

CREATE OR REPLACE FUNCTION check_result_value_uri()
RETURNS TRIGGER AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON observation
FOR EACH ROW
EXECUTE FUNCTION check_result_value_uri();

-- Trigger per il controllo del valore del risultato durante l'INSERT
CREATE OR REPLACE FUNCTION control_result_value() RETURNS TRIGGER AS
$$
BEGIN
    IF NEW.result_value > (
...
BEFORE INSERT OR UPDATE ON observation
FOR EACH ROW
EXECUTE FUNCTION control_result_value();
...
COMMENT ON TABLE parameter IS 'Describe an arbitrary event-specific parameter. This might be an environmental parameter, an instrument setting or input, or an event-specific sampling parameter that is not tightly bound to either the feature-of-interest  or to the observation procedure.';
-- Add Comment Column
COMMENT ON COLUMN parameter.id IS 'Primary Key of the Table';
COMMENT ON COLUMN parameter.name IS 'GenericName shall indicate the meaning of the named value. Its value should be taken from a well-governed source if possible.';
COMMENT ON COLUMN parameter.value IS 'Any shall provide the value. The type Any should be substituted by a suitable concrete type, such as CI_ResponsibleParty or Measure';
COMMENT ON COLUMN parameter.idobservation IS 'Foreign key to the Observation table, guidkey field.';
...
COMMENT ON TABLE processparameter IS 'Description of the given parameter.';
-- Add Comment Column
COMMENT ON COLUMN processparameter.id IS 'Primary Key of the Table';
COMMENT ON COLUMN processparameter.name IS 'Name of the process parameter.';
COMMENT ON COLUMN processparameter.description IS 'Description of the process parameter.';
COMMENT ON COLUMN processparameter.idprocess IS 'Foreign key to the Process table, guidkey field.';
...
COMMENT ON TABLE codelist IS 'Table to manage Codelists.';
-- Add Comment Column
COMMENT ON COLUMN codelist.id IS 'Uri of the codelist value';
COMMENT ON COLUMN codelist.label IS 'Label of the value';
COMMENT ON COLUMN codelist.definition IS 'Definition of the value';
COMMENT ON COLUMN codelist.collection IS 'Collection of values';
COMMENT ON COLUMN codelist.foi IS 'Feature of Interest of the collection';
COMMENT ON COLUMN codelist.phenomenon IS 'Phenomenon of the collection';
COMMENT ON COLUMN codelist.foi_phenomenon IS 'Concatenation of fields foi and phenomenon e';
...
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Abruptic', 'Abruptic', 'Abruptic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Aceric', 'Aceric', 'Aceric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Acroxic', 'Acroxic', 'Acroxic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Albic', 'Albic', 'Albic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Alcalic', 'Alcalic', 'Alcalic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Alumic', 'Alumic', 'Alumic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Anthric', 'Anthric', 'Anthric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Arenic', 'Arenic', 'Arenic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Aridic', 'Aridic', 'Aridic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Brunic', 'Brunic', 'Brunic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Calcaric', 'Calcaric', 'Calcaric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Carbonatic', 'Carbonatic', 'Carbonatic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Chloridic', 'Chloridic', 'Chloridic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Chromic', 'Chromic', 'Chromic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Clayic', 'Clayic', 'Clayic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Colluvic', 'Colluvic', 'Colluvic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Densic', 'Densic', 'Densic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Drainic', 'Drainic', 'Drainic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Dystric', 'Dystric', 'Dystric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Endoduric', 'Endoduric', 'Endoduric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Endoeutric', 'Endoeutric', 'Endoeutric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Epidystric', 'Epidystric', 'Epidystric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Epieutric', 'Epieutric', 'Epieutric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Escalic', 'Escalic', 'Escalic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Eutric', 'Eutric', 'Eutric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Ferric', 'Ferric', 'Ferric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Fragic', 'Fragic', 'Fragic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Gelic', 'Gelic', 'Gelic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Geric', 'Geric', 'Geric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Glossalbic', 'Glossalbic', 'Glossalbic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Glossic', 'Glossic', 'Glossic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Greyic', 'Greyic', 'Greyic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Gypsiric', 'Gypsiric', 'Gypsiric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Haplic', 'Haplic', 'Haplic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Hortic', 'Hortic', 'Hortic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Humic', 'Humic', 'Humic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Hyperdystric', 'Hyperdystric', 'Hyperdystric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Hypereutric', 'Hypereutric', 'Hypereutric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Hyperochric', 'Hyperochric', 'Hyperochric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Hyposalic', 'Hyposalic', 'Hyposalic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Hyposodic', 'Hyposodic', 'Hyposodic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Lamellic', 'Lamellic', 'Lamellic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Laxic', 'Laxic', 'Laxic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Magnesic', 'Magnesic', 'Magnesic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Manganiferric', 'Manganiferric', 'Manganiferric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Mesotrophic', 'Mesotrophic', 'Mesotrophic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Novic', 'Novic', 'Novic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Nudiargic', 'Nudiargic', 'Nudiargic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Ornithic', 'Ornithic', 'Ornithic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Oxyaquic', 'Oxyaquic', 'Oxyaquic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Pachic', 'Pachic', 'Pachic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Pellic', 'Pellic', 'Pellic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Petrogleyic', 'Petrogleyic', 'Petrogleyic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Pisocalcic', 'Pisocalcic', 'Pisocalcic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Placic', 'Placic', 'Placic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Plaggic', 'Plaggic', 'Plaggic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Profondic', 'Profondic', 'Profondic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Protothionic', 'Protothionic', 'Protothionic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Reductaquic', 'Reductaquic', 'Reductaquic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Reductic', 'Reductic', 'Reductic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Rhodic', 'Rhodic', 'Rhodic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Ruptic', 'Ruptic', 'Ruptic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Siltic', 'Siltic', 'Siltic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Skeletic', 'Skeletic', 'Skeletic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Sodic', 'Sodic', 'Sodic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Sombric', 'Sombric', 'Sombric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Sulphatic', 'Sulphatic', 'Sulphatic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Takyric', 'Takyric', 'Takyric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Tephric', 'Tephric', 'Tephric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Terric', 'Terric', 'Terric', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Thaptobathyfragic', 'Thaptobathyfragic', 'Thaptobathyfragic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Thionic', 'Thionic', 'Thionic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Thixotropic', 'Thixotropic', 'Thixotropic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Toxic', 'Toxic', 'Toxic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Transportic', 'Transportic', 'Transportic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Turbic', 'Turbic', 'Turbic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Umbriglossic', 'Umbriglossic', 'Umbriglossic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Vermic', 'Vermic', 'Vermic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Xanthic', 'Xanthic', 'Xanthic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Yermic', 'Yermic', 'Yermic', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('http://inspire.ec.europa.eu/codelist/WRBQualifierValue/Bathi', 'Bathi', 'Bathi', 'WRBQualifierValue', NULL, NULL, NULL, NULL);
...
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Bathi', 'Bathi', 'Bathi', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Cumuli', 'Cumuli', 'Cumuli', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Endo', 'Endo', 'Endo', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Epi', 'Epi', 'Epi', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Hyper', 'Hyper', 'Hyper', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Hypo', 'Hypo', 'Hypo', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Ortho', 'Ortho', 'Ortho', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Para', 'Para', 'Para', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Proto', 'Proto', 'Proto', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
INSERT INTO codelist (id, label, definition, collection, foi, phenomenon, foi_phenomenon, parent) VALUES ('https://crea.gov.it/infosuoli/vocabularies/WRBSpecifiers/Thapto', 'Thapto', 'Thapto', 'WRBSpecifierValue', NULL, NULL, NULL, NULL);
